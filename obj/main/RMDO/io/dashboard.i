...
# 62 "./src/main/io/dashboard.c" 2
# 1 "./src/main/io/displayport_oled.h" 1
# 18 "./src/main/io/displayport_oled.h"
       

displayPort_t *displayPortOledInit(void *device);
# 63 "./src/main/io/dashboard.c" 2
...
# 72 "./src/main/io/dashboard.c" 2



extern const dashboardConfig_t pgResetTemplate_dashboardConfig; dashboardConfig_t dashboardConfig_System; dashboardConfig_t dashboardConfig_Copy; extern const pgRegistry_t dashboardConfig_Registry; const pgRegistry_t dashboardConfig_Registry __attribute__ ((section(".pg_registry"), used, aligned(4))) = { .pgn = 519 | (0 << 12), .size = sizeof(dashboardConfig_t) | PGR_SIZE_SYSTEM_FLAG, .address = (uint8_t*)&dashboardConfig_System, .copy = (uint8_t*)&dashboardConfig_Copy, .ptr = 0, .reset = {.ptr = (void*)&pgResetTemplate_dashboardConfig}, };

const dashboardConfig_t pgResetTemplate_dashboardConfig __attribute__ ((section(".pg_resetdata"), used, aligned(2))) = { .device = ((I2CDEV_1) + 1), .address = 0x3C, }
...
static 
# 90 "./src/main/io/dashboard.c" 3 4
      _Bool 
# 90 "./src/main/io/dashboard.c"
           dashboardPresent = 
# 90 "./src/main/io/dashboard.c" 3 4
                              0
# 90 "./src/main/io/dashboard.c"
...
static char lineBuffer[(128 / (5 + 1)) + 1];
...
# 123 "./src/main/io/dashboard.c" 3 4
   _Bool 
# 123 "./src/main/io/dashboard.c"
        pageChanging;
...
# 196 "./src/main/io/dashboard.c"
static void updateTicker(void)
{
    static uint8_t tickerIndex = 0;
    i2c_OLED_set_xy(bus, (128 / (5 + 1)) - 1, 0);
    i2c_OLED_send_char(bus, tickerCharacters[tickerIndex]);
    tickerIndex++;
    tickerIndex = tickerIndex % (sizeof(tickerCharacters) / sizeof(char));
}

static void updateRxStatus(void)
{
    i2c_OLED_set_xy(bus, (128 / (5 + 1)) - 2, 0);
    char rxStatus = '!';
...
static void updateFailsafeStatus(void)
{
    ...
    i2c_OLED_set_xy(bus, (128 / (5 + 1)) - 3, 0);
...
static void drawRxChannel(uint8_t channelIndex, uint8_t width)
{
    ...
    const uint32_t percentage = (constrain(rcData[channelIndex], 1000, 2000) - 1000) * 100 / (2000 - 1000);
...
{
    for (int channelIndex = 0; channelIndex < rxRuntimeConfig.channelCount && channelIndex < 14; channelIndex += 2) {
        i2c_OLED_set_line(bus, (channelIndex / 2) + 1);

        drawRxChannel(channelIndex, ((128 / (5 + 1)) / 2));
        ...
        drawRxChannel(channelIndex + 1, ((128 / (5 + 1)) / 2));
    }
}
...
static void showWelcomePage(void)
{
    ...
    tfp_sprintf(lineBuffer, "v%s (%s)", "3" "." "2" "." "0", shortGitRevision);
...
static void showBatteryPage(void)
{
    ...
    if (batteryConfig()->voltageMeterSource != VOLTAGE_METER_NONE) {
        ...
        i2c_OLED_set_line(bus, rowIndex++);
        drawHorizonalPercentageBar((128 / (5 + 1)), batteryPercentage);
    }
    ...
    if (batteryConfig()->currentMeterSource != CURRENT_METER_NONE) {
        ...
        i2c_OLED_set_line(bus, rowIndex++);
        drawHorizonalPercentageBar((128 / (5 + 1)), capacityPercentage);
    }
}
...
# 501 "./src/main/io/dashboard.c"
    tfp_sprintf(lineBuffer, format, "I&H", attitude.values.roll, attitude.values.pitch, (attitude.values.yaw / 10));
    padLineBuffer();
    i2c_OLED_set_line(bus, rowIndex++);
    i2c_OLED_send_string(bus, lineBuffer);
# 532 "./src/main/io/dashboard.c"
}
...
static void showTasksPage(void)
{
    ...
    for (cfTaskId_e taskId = 0; taskId < TASK_COUNT; ++taskId) {
        ...
        if (taskInfo.isEnabled && taskId != TASK_SERIAL) {
...
static void showDebugPage(void)
{
    for (int rowIndex = 0; rowIndex < 4; rowIndex++) {
        ...
        padLineBuffer();
        i2c_OLED_set_line(bus, rowIndex + 1);
        i2c_OLED_send_string(bus, lineBuffer);
    }
}
...
static const pageEntry_t pages[PAGE_COUNT] = {
    { PAGE_WELCOME, "Betaflight", showWelcomePage, (1 << 0) },
    { PAGE_ARMED, "ARMED", showArmedPage, (1 << 0) },
    { PAGE_PROFILE, "PROFILE", showProfilePage, 0 },



    { PAGE_RX, "RX", showRxPage, 0 },
    { PAGE_BATTERY, "BATTERY", showBatteryPage, 0 },
    { PAGE_SENSORS, "SENSORS", showSensorsPage, 0 },

    { PAGE_TASKS, "TASKS", showTasksPage, 0 },


    { PAGE_DEBUG, "DEBUG", showDebugPage, 0 },
...
void dashboardUpdate(timeUs_t currentTimeUs)
{
    ...
    const 
# 614 "./src/main/io/dashboard.c" 3 4
         _Bool 
# 614 "./src/main/io/dashboard.c"
              updateNow = (int32_t)(currentTimeUs - nextDisplayUpdateAt) >= 0L;
    if (!updateNow) {
        return;
    }

    nextDisplayUpdateAt = currentTimeUs + ((1000 * 1000) / 5);

    
# 621 "./src/main/io/dashboard.c" 3 4
   _Bool 
# 621 "./src/main/io/dashboard.c"
        armedState = (armingFlags & (ARMED)) ? 
# 621 "./src/main/io/dashboard.c" 3 4
                                          1 
# 621 "./src/main/io/dashboard.c"
                                               : 
...
# 622 "./src/main/io/dashboard.c" 3 4
   _Bool 
# 622 "./src/main/io/dashboard.c"
        armedStateChanged = armedState != previousArmedState;
...
# 630 "./src/main/io/dashboard.c" 3 4
                                1
# 630 "./src/main/io/dashboard.c"
                                    ;
    } else {
        if (armedStateChanged) {
            pageState.pageFlags |= PAGE_STATE_FLAG_FORCE_PAGE_CHANGE;
        ...
        if (pageState.pageChanging && (pageState.pageFlags & PAGE_STATE_FLAG_CYCLE_ENABLED)) {
            ...
            } while (pageState.page->flags & (1 << 0));
        }
    }

    if (pageState.pageChanging) {
        pageState.pageFlags &= ~PAGE_STATE_FLAG_FORCE_PAGE_CHANGE;
        pageState.nextPageAt = currentTimeUs + ((1000 * 1000) * 5);
...
void dashboardInit(void)
{
    ...
    dashBoardBus.busdev_u.i2c.device = ((dashboardConfig()->device) - 1);
    ...
    dashboardSetNextPageChangeAt(now + ((1000 * 1000) * 5));
}
...
void dashboardResetPageCycling(void)
{
    pageState.cycleIndex = PAGE_COUNT - 1;
