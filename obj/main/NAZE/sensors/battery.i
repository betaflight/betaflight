...
const batteryConfig_t pgResetTemplate_batteryConfig __attribute__ ((section(".pg_resetdata"), used, aligned(2))) = { .vbatmaxcellvoltage = 43, .vbatmincellvoltage = 33, .vbatwarningcellvoltage = 35, .vbatnotpresentcellvoltage = 30, .voltageMeterSource = VOLTAGE_METER_ADC, .lvcPercentage = 100, .batteryCapacity = 0, .currentMeterSource = CURRENT_METER_NONE, .useVBatAlerts = 
...
void batteryUpdateCurrentMeter(timeUs_t currentTimeUs)
{
    ...
    switch (batteryConfig()->currentMeterSource) {
        ...
        case CURRENT_METER_VIRTUAL: {
# 395 "./src/main/sensors/battery.c"
            break;
        }
