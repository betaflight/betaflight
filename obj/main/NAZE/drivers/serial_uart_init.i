...
# 37 "./src/main/drivers/serial_uart_init.c" 2
...
# 48 "./src/main/drivers/serial_uart_init.c"
    ...
    if (inverted) {
        ...
        enableInverter(uartPort->USARTx, 
# 53 "./src/main/drivers/serial_uart_init.c" 3 4
                                        1
# 53 "./src/main/drivers/serial_uart_init.c"
                                            );
    }
# 70 "./src/main/drivers/serial_uart_init.c"
}
...
# 84 "./src/main/drivers/serial_uart_init.c" 3 4
           0 
# 84 "./src/main/drivers/serial_uart_init.c"
                 &&
    ...
    USART_InitStructure.USART_StopBits = (uartPort->port.options & SERIAL_STOPBITS_2) ? ((uint16_t)0x2000) : ((uint16_t)0x0000);
    USART_InitStructure.USART_Parity = (uartPort->port.options & SERIAL_PARITY_EVEN) ? ((uint16_t)0x0400) : ((uint16_t)0x0000);
...
# 121 "./src/main/drivers/serial_uart_init.c"
    ...
    if (mode & MODE_RX) {
# 151 "./src/main/drivers/serial_uart_init.c"
        if (s->rxDMAChannel) {
...
# 174 "./src/main/drivers/serial_uart_init.c"
            ...
            USART_DMACmd(s->USARTx, ((uint16_t)0x0040), ENABLE);
            s->rxDMAPos = DMA_GetCurrDataCounter(s->rxDMAChannel);

        } else {
            USART_ClearITPendingBit(s->USARTx, ((uint16_t)0x0525));
            USART_ITConfig(s->USARTx, ((uint16_t)0x0525), ENABLE);
        }
    }


    if (mode & MODE_TX) {
# 205 "./src/main/drivers/serial_uart_init.c"
        if (s->txDMAChannel) {
            DMA_StructInit(&DMA_InitStructure);
...
# 226 "./src/main/drivers/serial_uart_init.c"
            ...
            USART_DMACmd(s->USARTx, ((uint16_t)0x0080), ENABLE);
        } else {
            USART_ITConfig(s->USARTx, ((uint16_t)0x0727), ENABLE);
        }
    }
