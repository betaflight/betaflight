...
static void __usbd_event_ep0_setup_complete_handler(uint8_t busid, struct usb_setup_packet *setup)
{
    ...
    if (setup->wLength && ((setup->bmRequestType & USB_REQUEST_DIR_MASK) == USB_REQUEST_DIR_OUT)) {
		...
		extern int usbd_ep0_read(uint8_t *data, uint32_t data_len);
		usbd_ep0_read(g_usbd_core[busid].ep0_data_buf, setup->wLength);
        return;
    }
	...
	if (autoSetAddr)
	{
		...
		extern int usbd_set_addr_status(const uint8_t *data);
		usbd_set_addr_status(g_usbd_core[busid].ep0_data_buf);
	}
	else
	{
