...
static bool SCSI_modeSense6(uint8_t busid, uint8_t **data, uint32_t *len)
{
    ...
    uint8_t sense6[SCSIRESP_MODEPARAMETERHDR6_SIZEOF] = { 0x03, 0x00, 0x00, 0x00 }; //�޸�Ϊд����ʹ��{ 0x03, 0x00, 0x80, 0x00 }
...
#endif
uint8_t USBD_MSC_RegisterStorage(USBD_StorageTypeDef *pdev, USBD_StorageTypeDef *fops)
{
  if (fops == NULL)
  {
    return (uint8_t)1;
