...
#error "ERROR: usbd_dfu_core.c: Modify the file to support more descriptors!"
  ...
  0x1A,                                /* bcdDFUVersion = 0x011A (DfuSe extension) */
  0x01
  /***********************************************************/
  /* 9*/
};
...
static void DFU_Upload(USBD_HandleTypeDef *pdev, USBD_SetupReqTypedef *req)
{
  ...
  if (req->wLength > 0U)
  {
    /* Betaflight patch — also accept DFU_STATE_DNLOAD_IDLE so that a
     * DfuSe SET_ADDR (DOWNLOAD wValue=0,wLength=5,data=[0x21,addr])
     * followed by an UPLOAD doesn't NAK. The stock state machine drops
     * into DNLOAD_IDLE after the SET_ADDR + GET_STATUS handshake;
     * dfu-util then issues UPLOAD without a prior ABORT. */
    if ((hdfu->dev_state == DFU_STATE_IDLE)
            || (hdfu->dev_state == DFU_STATE_UPLOAD_IDLE)
            || (hdfu->dev_state == DFU_STATE_DNLOAD_IDLE))
    {
...
/**
  ...
  * @retval None
  */
/* Betaflight extension — set by DFU_Leave when manifestation completes.
 * obl_app.c's loop polls this and triggers NVIC_SystemReset so the host
 * doesn't have to manually reset the board to run the freshly-flashed
 * BF. Volatile because it's written from the USB EP0 TxReady ISR path
 * and read from the run_obl_loop main-loop polling. */
volatile bool dfu_leave_pending;
