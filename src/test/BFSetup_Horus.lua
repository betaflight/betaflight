...
local SetupPages = {
   ...
   },
   {
      ...
      fields = {
         ...
         -- Roll + Pitch
         { t = "Rate",  x = 152, y = 80, sp = 60, i=1 },
         { t = "Expo",  x = 264, y = 80, sp = 60, i=2 },

         -- Yaw
         { t = "Yaw",   x = 20,  y = 128, sp = 76, i=5 },
         { t = "Rate",  x = 152, y = 128, sp = 60, i=12 },
         { t = "Expo",  x = 264, y = 128, sp = 60, i=11 },
      },
      read  = MSP_RC_TUNING,
...
local function drawScreen(page,page_locked)
   ...
   drawScreenTitle('Betaflight setup: '..screen_title,currentPage,#(SetupPages))
   ...
   for i=1,#(page.fields) do
      ...
      lcd.drawText(f.x, f.y, f.t .. ":", TEXT_COLOR)
...
local function drawMenu()
   ...
   local y = 100
   local w = 200
   local h = #(menuList) * 20 + 12
   lcd.drawFilledRectangle(x,y,w,h,TEXT_BGCOLOR)
   lcd.drawRectangle(x,y,w-1,h-1,LINE_COLOR)
   lcd.drawText(x+4,y+3,"Menu:",TEXT_COLOR)

   for i,e in ipairs(menuList) do
      if menuActive == i then
         lcd.drawText(x+68,y+(i-1)*20 +6, e.t, TEXT_COLOR + INVERS)
      else
         lcd.drawText(x+68,y+(i-1)*20 +6, e.t, TEXT_COLOR)
      end
   end
end
...
local function run(event)
   ...
   elseif gState <= PAGE_DISPLAY then
      if event == EVT_PAGEUP_FIRST then
         incPage(-1)
      elseif event == EVT_PAGEDN_FIRST then
         incPage(1)
      elseif event == EVT_PLUS_BREAK or event == EVT_ROT_LEFT then
         incLine(-1)
      elseif event == EVT_MINUS_BREAK or event == EVT_ROT_RIGHT then
   ...
   elseif gState == PAGE_SAVING then
      ...
      lcd.drawRectangle(120,100,180,60, LINE_COLOR)
      lcd.drawText(132,112,"Saving...", DBLSIZE + BLINK + TEXT_COLOR)
   end
