...
// Yaw control modes for multirotor
typedef enum {
    YAW_MODE_VELOCITY = 0,  // Point nose in velocity direction
    YAW_MODE_BEARING = 1,   // Point nose at waypoint
    YAW_MODE_HYBRID = 2,    // Blend based on distance
    YAW_MODE_FIXED = 3,     // No yaw control
    YAW_MODE_COUNT
} autopilotYawMode_e;
...
typedef struct autopilotConfig_s {
    ...
    uint16_t minYawVelocity;          // cm/s, minimum velocity for velocity-based yaw (default 100)
    ...
    uint8_t maxTurnRate;              // deg/s, maximum turn rate (default 3 for rate-1 turn, configurable up to 30)
