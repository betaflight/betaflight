...
static bool testBlackboxConditionUncached(FlightLogFieldCondition condition)
{
    ...
    case CONDITION(MOTOR_8_HAS_RPM):
        return (getMotorCount() >= condition - CONDITION(MOTOR_1_HAS_RPM) + 1) && isDshotTelemetryEnabled() && isFieldEnabled(FIELD_SELECT(RPM));
#endif
...
#endif
#ifdef USE_DSHOT_TELEMETRY
        BLACKBOX_PRINT_HEADER_LINE(PARAM_NAME_DSHOT_BIDIR, "%d",            isDshotTelemetryEnabled());
