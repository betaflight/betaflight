...
static void validateAndFixConfig(void)
{
    ...
    if (!rcSmoothingIsEnabled() || rxConfig()->rcSmoothingChannels == SMOOTHING_CHANNELS_T) {
        for (unsigned i = 0; i < PID_PROFILE_COUNT; i++) {
    ...
    if (!rcSmoothingIsEnabled() ||
        (rxConfig()->rcSmoothingChannels != SMOOTHING_CHANNELS_RPY &&
         rxConfig()->rcSmoothingChannels != SMOOTHING_CHANNELS_RPYT)) {
    ...
    if (!rcSmoothingIsEnabled() ||
        !(rxConfig()->rcSmoothingChannels == SMOOTHING_CHANNELS_RPYT
        || rxConfig()->rcSmoothingChannels == SMOOTHING_CHANNELS_T
        || rxConfig()->rcSmoothingChannels == SMOOTHING_CHANNELS_RPT)) {
        for (unsigned i = 0; i < PID_PROFILE_COUNT; i++) {
