...
// Scaling factors for Pids for better tunable range in configurator for betaflight pid controller. The scaling is based
// on legacy pid controller or previous float
...
typedef struct pidProfile_s {
    ...
    uint8_t pidAtMinThrottle; // Disable/Enable pids on zero throttle. Normally even without airmode P and D would be
                              // active.
    ...
    uint8_t horizon_ignore_sticks; // 0 = default, meaning both stick and attitude attenuation; 1 = only attitude
                                   // attenuation
    ...
    uint16_t crash_setpoint_threshold; // setpoint must be below this value to detect crash, so flips and rolls are not
    ...
    uint8_t throttle_boost; // how much should throttle be boosted during transient changes 0-100, 100 adds 10x hpf
                            // filtered throttle
    uint8_t throttle_boost_cutoff; // Which cutoff frequency to use for throttle boost. higher cutoffs keep the boost on
                                   // for shorter. Specified in hz.
    uint8_t iterm_rotation; // rotates iterm to translate world errors to local coordinate system
    uint8_t iterm_relax_type; // Specifies type of relax algorithm
    uint8_t iterm_relax_cutoff; // This cutoff frequency specifies a low pass filter which predicts average response of
                                // the quad to setpoint
    ...
    uint8_t acro_trainer_gain; // The strength of the limiting. Raising may reduce overshoot but also lead to
                               // oscillation around the angle limit
    ...
    uint8_t launchControlMode; // Whether launch control is limited to pitch only (launch stand or top-mount) or all
                               // axes (on battery)
    ...
    uint8_t integrated_yaw_relax; // Specifies how much integrated yaw should be reduced to offset the drag based yaw
                                  // component
    ...
    uint8_t
        feedforward_yaw_hold_gain; // Amount of sustained high-pass yaw setpoint to add to feedforward, zero disables
    uint8_t feedforward_yaw_hold_time; // Time constant of the sustained yaw hold element in ms to add to feed forward,
    ...
    uint8_t tpa_low_always; // off, on - if OFF then low TPA is only active until tpa_low_breakpoint is reached the
                            // first time
    ...
    uint16_t tpa_speed_basic_delay; // For wings when tpa_speed_type = BASIC: delay of air speed estimation from
                                    // throttle in milliseconds (time of reaching 50% of terminal speed in horizontal
                                    // flight at full throttle)
    uint16_t tpa_speed_basic_gravity; // For wings when tpa_speed_type = BASIC: gravity effect on air speed estimation
                                      // in percents
    ...
    uint16_t tpa_speed_max_voltage; // For wings: theoretical max voltage; used for throttle scailing with voltage for
                                    // air speed estimation
    ...
    int16_t angle_pitch_offset; // For wings: pitch offset for angle modes; in decidegrees; positive values tilting the
                                // wing down
...
typedef struct pidConfig_s {
    ...
    uint16_t runaway_takeoff_deactivate_delay; // delay in ms for "in-flight" conditions before deactivation (successful
                                               // flight)
...
void applyItermRelax(const int axis, const float iterm, const float gyroRate, float *itermErrorRate,
                     float *currentPidSetpoint);
void applyAbsoluteControl(const int axis, const float gyroRate, float *currentPidSetpoint, float *itermErrorRate);
void rotateItermAndAxisError();
float pidLevel(int axis, const pidProfile_t *pidProfile, const rollAndPitchTrims_t *angleTrim, float rawSetpoint,
