...
// Note that lower max requires more samples to be averaged, increasing precision but taking longer to get enough
// samples. For Bosch at 3200Hz gyro, max of 600, int(3200/1200) = 2, sdftSampleRateHz = 1600, range to 800hz. For Bosch
// on XClass, better to set a max of 300, int(3200/600) = 5, sdftSampleRateHz = 640, range to 320Hz.
...
// The SDFT code is split into steps. It takes 4 PID loops to calculate the SDFT, track peaks and update the filters for
// one axis. Since there are three axes, it takes 12 PID loops to completely update all axes. At 8k, any one axis gets
// updated at 8000 / 12 or 666hz or every 1.5ms In this time, 2 points in the SDFT buffer will have changed. At 4k, it
// takes twice as long to update an axis, i.e. each axis updates only every 3ms. Four points in the buffer will have
// changed in that time, and each point will be the average of three samples. Hence output jitter at 4k is about four
// times worse than at 8k. At 2k output jitter is quite bad.
...
void dynNotchInit(const dynNotchConfig_t *config, const timeUs_t targetLooptimeUs)
{
    ...
    sdftEndBin =
        MIN(SDFT_BIN_COUNT - 1, lrintf(dynNotch.maxHz / sdftResolutionHz)); // can't use more than SDFT_BIN_COUNT bins.
    ...
    for (int axis = 0; axis < XYZ_AXIS_COUNT; axis++) {
        for (int p = 0; p < dynNotch.count; p++) {
            // any init value is fine, but evenly spreading centerFreqs across frequency range makes notches stick to
            // peaks quicker
            dynNotch.centerFreq[axis][p] =
                (p + 0.5f) * (dynNotch.maxHz - dynNotch.minHz) / (float)dynNotch.count + dynNotch.minHz;
            biquadFilterInit(&dynNotch.notch[axis][p], dynNotch.centerFreq[axis][p], dynNotch.looptimeUs, dynNotch.q,
                             FILTER_NOTCH, 1.0f);
        }
    }
}
...
static FAST_CODE_NOINLINE void dynNotchProcess(void)
{
    ...
    case STEP_CALC_FREQUENCIES: // 4.0us (2-7us) @ F722
    {
        ...
        for (int p = 0; p < dynNotch.count; p++) {
            ...
            if (peaks[p].bin != 0 && peaks[p].value > sdftNoiseThreshold) {
                ...
                // PT1 style smoothing moves notch center freqs rapidly towards big peaks and slowly away, up to 10x
                // faster
    ...
    {
        for (int p = 0; p < dynNotch.count; p++) {
            // Only update notch filter coefficients if the corresponding peak got its center frequency updated in the
            // previous step
            if (peaks[p].bin != 0 && peaks[p].value > sdftNoiseThreshold) {
                biquadFilterUpdate(&dynNotch.notch[state.axis][p], dynNotch.centerFreq[state.axis][p],
                                   dynNotch.looptimeUs, dynNotch.q, FILTER_NOTCH, 1.0f);
            }
        }
