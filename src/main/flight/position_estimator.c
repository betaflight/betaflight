...
void positionEstimatorInit(void)
{
    ...
    estimate.targetPosition = (vector3_t){{0, 0, 0}};
...
void positionEstimatorSetTargetToCurrentXY(void)
{
    estimate.targetPosition.x = estimate.position.x;
    estimate.targetPosition.y = estimate.position.y;
}

void positionEstimatorSetTargetToCurrentZ(void)
{
    estimate.targetPosition.z = estimate.position.z;
}

void positionEstimatorSetTargetXY(float eastCm, float northCm)
{
    estimate.targetPosition.x = eastCm;
    estimate.targetPosition.y = northCm;
}

void positionEstimatorSetTargetZ(float upCm)
{
    estimate.targetPosition.z = upCm;
}
...
void positionEstimatorResetZ(void)
{
    ...
    estimate.targetPosition.z = 0.0f;
    estimate.isValidZ = false;
    lastZMeasurementUs = 0;
#ifdef USE_GPS
    gpsAltOffsetCm = 0.0f;
...
void positionEstimatorResetXY(void)
{
    ...
    estimate.targetPosition.x = 0.0f;
    estimate.targetPosition.y = 0.0f;
    estimate.isValidXY = false;
    lastXYMeasurementUs = 0;
#ifdef USE_GPS
    if (sensors(SENSOR_GPS) && STATE(GPS_FIX)) {
