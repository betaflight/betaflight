...
static bool positionEstimatorWantXYFusion(void)
{
    ...
    if (sensors(SENSOR_OPTICALFLOW) && sensors(SENSOR_RANGEFINDER) && isOpticalflowHealthy() &&
...
static void feedGPSMeasurements(timeUs_t nowUs)
{
    ...
    const bool gpsAltAllowed = (altSource == ALTITUDE_SOURCE_DEFAULT || altSource == ALTITUDE_SOURCE_GPS_ONLY ||
...
void positionEstimatorUpdate(void)
{
    ...
    if (xyEnabled) {
        estimate.isValidXY =
            (lastXYMeasurementUs > 0) && (cmpTimeUs(nowUs, lastXYMeasurementUs) < MEASUREMENT_TIMEOUT_US);
    } else {
        estimate.isValidXY = false;
    }
