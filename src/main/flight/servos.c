...
static FILTER_TYPE(biquad, MAX_SUPPORTED_SERVOS)  servoFilter;

void servosFilterInit(void)
{
    if (servoConfig()->servo_lowpass_freq) {
        FILTER_INIT_FN_NAME(biquad, MAX_SUPPORTED_SERVOS)(&servoFilter);
        biquadFilterCoeffsLPF(&servoFilter.coeffs, servoConfig()->servo_lowpass_freq, targetPidLooptime * 1e-6f);
    }
...
static void filterServos(void)
{
    ...
    if (servoConfig()->servo_lowpass_freq) {
        for (int servoIdx = 0; servoIdx < MAX_SUPPORTED_SERVOS; servoIdx++) {
            //TODO: use float array for servos
            servo[servoIdx] = lrintf(FILTER_APPLY_AXIS_FN_NAME(biquad, MAX_SUPPORTED_SERVOS)(&servoFilter, servo[servoIdx], servoIdx));
            // Sanity check
