...
void servoMixer(void)
{
    ...
    if (FLIGHT_MODE(PASSTHRU_MODE)) {
        ...
        // Reverse yaw servo when inverted in 3D mode. Reads the mixer's actual selected motor
        // direction rather than raw stick position, so this stays correct during GPS Rescue
        // (which picks direction from measured tilt, not the stick) and crashflip.
