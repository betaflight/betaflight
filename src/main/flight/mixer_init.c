...
// All PWM motor scaling is done to standard PWM range of 1000-2000 for easier tick conversion with legacy code /
// configurator DSHOT scaling is done to the actual dshot range
void initEscEndpoints(void)
{
    ...
    motorInitEndpoints(motorConfig(), motorOutputLimit, &mixerRuntime.motorOutputLow, &mixerRuntime.motorOutputHigh,
                       &mixerRuntime.disarmMotorOutput, &mixerRuntime.deadbandMotor3dHigh,
                       &mixerRuntime.deadbandMotor3dLow);
}
...
void mixerInitProfile(void)
{
    ...
    if (!mixerRuntime.feature3dEnabled && mixerRuntime.dynIdleMinRps) {
        mixerRuntime.motorOutputLow =
            DSHOT_MIN_THROTTLE; // Override value set by initEscEndpoints to allow zero motor drive
    }
#endif
    ...
    if (currentPidProfile->vbat_sag_compensation > 0 && !RPM_LIMIT_ACTIVE) {
        ...
        mixerRuntime.vbatRangeToCompensate =
            currentBatteryProfile ? mixerRuntime.vbatFull - currentBatteryProfile->vbatwarningcellvoltage : 0;
...
{
    mixerRuntime.rpmLimiterI = 0.0f;
    mixerRuntime.rpmLimiterThrottleScale =
        constrainf(mixerRuntime.rpmLimiterRpmLimit / motorEstimateMaxRpm(), 0.0f, 1.0f);
