...
#define ALTITUDE_INNER_I_SCALE                                                                                         \
    0.00012f // scale on altitudeI for inner integral (throttle, similar order to legacy ALTITUDE_I_SCALE)
...
void autopilotCaptureHoverThrottleForAltHold(void)
{
    ...
    altHoldCapturedHoverPwm = (uint16_t)lrintf(
        constrainf(rcCommand[THROTTLE], (float)AP_HOVER_THROTTLE_CAPTURE_MIN, (float)AP_HOVER_THROTTLE_CAPTURE_MAX));
}
...
void altitudeControl(float targetAltitudeCm, float taskIntervalS, float targetAltitudeVelCmS, float velLimitCmS)
{
    ...
    altitudeVelIntegral =
        constrainf(altitudeVelIntegral, -ALTITUDE_INNER_IWINDUP_THROTTLE, ALTITUDE_INNER_IWINDUP_THROTTLE);
