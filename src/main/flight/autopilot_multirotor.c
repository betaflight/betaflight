...
void resetPositionControl(unsigned taskRateHz)
{
    ...
    // Tell the estimator to enable XY and set target to current position
    positionEstimatorEnableXY(true);
    positionEstimatorSetTargetToCurrentXY();
}
...
bool positionControl(void)
{
    ...
    const float errorEast  = est->position.x - est->targetPosition.x;
    const float errorNorth = est->position.y - est->targetPosition.y;
