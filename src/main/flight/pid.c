...
PG_RESET_TEMPLATE(pidConfig_t, pidConfig, .pid_process_denom = DEFAULT_PID_PROCESS_DENOM,
...
#define CRASH_RECOVERY_DETECTION_DELAY_US                                                                              \
    1000000 // 1 second delay before crash recovery detection is active after entering a self-level mode
...
void resetPidProfile(pidProfile_t *pidProfile)
{
    RESET_CONFIG(
        ...
        .pidSumLimit = PIDSUM_LIMIT, .pidSumLimitYaw = PIDSUM_LIMIT_YAW, .yaw_lowpass_hz = 100, .dterm_notch_hz = 0,
        .dterm_notch_cutoff = 0,
        .itermWindup = 80, // sets iTerm limit to this percentage below pidSumLimit
        .pidAtMinThrottle = PID_STABILISATION_ON, .angle_limit = 60, .feedforward_transition = 0,
        .yawRateAccelLimit = 0, .rateAccelLimit = 0, .anti_gravity_gain = 80,
        .crash_time = 500, // ms
        .crash_delay = 0, // ms
        ...
        .horizon_limit_degrees = 135, .horizon_ignore_sticks = false, .crash_limit_yaw = 200, .itermLimit = 400,
        .throttle_boost = 5, .throttle_boost_cutoff = 15, .iterm_rotation = false, .iterm_relax = ITERM_RELAX_RP,
        .iterm_relax_cutoff = ITERM_RELAX_CUTOFF_DEFAULT, .iterm_relax_type = ITERM_RELAX_SETPOINT,
        .acro_trainer_angle_limit = 20, .acro_trainer_lookahead_ms = 50, .acro_trainer_debug_axis = FD_ROLL,
        .acro_trainer_gain = 75, .abs_control_gain = 0, .abs_control_limit = 90, .abs_control_error_limit = 20,
        .abs_control_cutoff = 11, .dterm_lpf1_static_hz = DTERM_LPF1_DYN_MIN_HZ_DEFAULT,
        ...
        .dterm_lpf1_dyn_min_hz = DTERM_LPF1_DYN_MIN_HZ_DEFAULT, .dterm_lpf1_dyn_max_hz = DTERM_LPF1_DYN_MAX_HZ_DEFAULT,
        .launchControlMode = LAUNCH_CONTROL_MODE_PITCHONLY, .launchControlThrottlePercent = 20,
        .launchControlAngleLimit = 0, .launchControlGain = 40, .launchControlAllowTriggerReset = true,
        .use_integrated_yaw = false, .integrated_yaw_relax = 200, .thrustLinearization = 0, .d_max = D_MAX_DEFAULT,
        .d_max_gain = 37, .d_max_advance = 20, .motor_output_limit = 100,
        .auto_profile_cell_count = AUTO_PROFILE_CELL_COUNT_STAY, .profileName = {0}, .dyn_idle_min_rpm = 0,
        .dyn_idle_p_gain = 50, .dyn_idle_i_gain = 50, .dyn_idle_d_gain = 50, .dyn_idle_max_increase = 150,
        .feedforward_averaging = FEEDFORWARD_AVERAGING_2_POINT, .feedforward_max_rate_limit = 90,
        .feedforward_smooth_factor = 65, .feedforward_jitter_factor = 7, .feedforward_boost = 15,
        .dterm_lpf1_dyn_expo = 5, .level_race_mode = false, .vbat_sag_compensation = 0,
        .simplified_pids_mode = PID_SIMPLIFIED_TUNING_RPY, .simplified_master_multiplier = SIMPLIFIED_TUNING_DEFAULT,
        .simplified_roll_pitch_ratio = SIMPLIFIED_TUNING_DEFAULT, .simplified_i_gain = SIMPLIFIED_TUNING_DEFAULT,
        .simplified_d_gain = SIMPLIFIED_TUNING_D_DEFAULT, .simplified_pi_gain = SIMPLIFIED_TUNING_DEFAULT,
        .simplified_d_max_gain = SIMPLIFIED_TUNING_D_DEFAULT, .simplified_feedforward_gain = SIMPLIFIED_TUNING_DEFAULT,
        .simplified_pitch_pi_gain = SIMPLIFIED_TUNING_DEFAULT, .simplified_dterm_filter = true,
        .simplified_dterm_filter_multiplier = SIMPLIFIED_TUNING_DEFAULT, .anti_gravity_cutoff_hz = 5,
        .anti_gravity_p_gain = 100, .tpa_mode = TPA_MODE_D, .tpa_rate = 65, .tpa_breakpoint = 1350,
        ...
        .tpa_low_rate = 20, .tpa_low_breakpoint = 1050, .tpa_low_always = 0, .ez_landing_threshold = 25,
        .ez_landing_limit = 15, .ez_landing_speed = 50, .spa_center = {0, 0, 0}, .spa_width = {0, 0, 0},
        .spa_mode = {0, 0, 0},
        ...
        .feedforward_yaw_hold_time = 100, // a value of 100 is a time constant of about 100ms, and is OK for a 5in;
                                          // smaller values decay faster, eg for smaller props
        .tpa_curve_type = TPA_CURVE_CLASSIC, .tpa_curve_stall_throttle = 30, .tpa_curve_pid_thr0 = 200,
        .tpa_curve_pid_thr100 = 70, .tpa_curve_expo = 20, .tpa_speed_type = TPA_SPEED_BASIC,
        .tpa_speed_basic_delay = 1000, .tpa_speed_basic_gravity = 50, .tpa_speed_adv_prop_pitch = 370,
        .tpa_speed_adv_mass = 1000, .tpa_speed_adv_drag_k = 1000, .tpa_speed_adv_thrust = 2000,
        .tpa_speed_max_voltage = 2520, .tpa_speed_pitch_offset = 0, .yaw_type = YAW_TYPE_RUDDER,
        .angle_pitch_offset = 0, .chirp_lag_freq_hz = 3, .chirp_lead_freq_hz = 30, .chirp_amplitude_roll = 230,
        .chirp_amplitude_pitch = 230, .chirp_amplitude_yaw = 180, .chirp_frequency_start_deci_hz = 2,
...
static float calcWingAcceleration(float throttle, float pitchAngleRadians)
{
    ...
    const float thrust =
        (throttle * throttle - throttle * tpa->speed * tpa->inversePropMaxSpeed) * tpa->twr * G_ACCELERATION;
...
static float calcWingTpaArgument(void)
{
    ...
    const float correctedPitchAnge =
        pitchRadians + cos_approx(pitchRadians) * cos_approx(rollRadians) * pidRuntime.tpaSpeed.pitchOffset;
...
static float getTpaFactorClassic(float tpaArgument)
{
    ...
    bool isThrottlePastTpaLowBreakpoint =
        (tpaArgument >= pidRuntime.tpaLowBreakpoint || pidRuntime.tpaLowBreakpoint <= 0.01f);
    float tpaRate = 0.0f;
...
STATIC_UNIT_TESTED FAST_CODE_NOINLINE float calcHorizonLevelStrength(void)
{
    ...
    float horizonLevelStrength =
        MAX((pidRuntime.horizonLimitDegrees - currentInclination) * pidRuntime.horizonLimitDegreesInv, 0.0f);
    // 1.0 when attitude is 'flat', 0 when angle is equal to, or greater than, horizonLimitDegrees
    horizonLevelStrength *=
        MAX((pidRuntime.horizonLimitSticks - absMaxStickDeflection) * pidRuntime.horizonLimitSticksInv,
...
STATIC_UNIT_TESTED FAST_CODE_NOINLINE float pidLevel(int axis, const pidProfile_t *pidProfile,
                                                     ...
                                                     float horizonLevelStrength)
{
    ...
    const float currentAngle =
        (attitude.raw[axis] - angleTrim->raw[axis]) / 10.0f; // stepped at 500hz with some 4ms flat spots
    ...
    // earthRef code here takes about 76 cycles, if conditional on angleEarthRef it takes about 100.  sin_approx costs
    ...
    if (axis == FD_ROLL) {
        ...
        DEBUG_SET(DEBUG_ANGLE_MODE, 1,
                  lrintf(errorAngle * pidRuntime.angleGain * 10.0f)); // un-smoothed error correction in degrees
...
static FAST_CODE_NOINLINE void handleCrashRecovery(const pidCrashRecovery_e crash_recovery,
                                                   ...
                                                   float *currentPidSetpoint, float *errorRate)
{
    if (pidRuntime.inCrashRecoveryMode &&
        cmpTimeUs(currentTimeUs, pidRuntime.crashDetectedAtUs) > pidRuntime.crashTimeDelayUs) {
        ...
        if (axis == FD_YAW) {
            *errorRate = constrainf(*errorRate, -pidRuntime.crashLimitYaw, pidRuntime.crashLimitYaw);
            ...
            // on roll and pitch axes calculate currentPidSetpoint and errorRate to level the aircraft to recover from
            // crash
        ...
        if (cmpTimeUs(currentTimeUs, pidRuntime.crashDetectedAtUs) > pidRuntime.crashTimeLimitUs ||
            (getMotorMixRange() < 1.0f && fabsf(gyro.gyroADCf[FD_ROLL]) < pidRuntime.crashRecoveryRate &&
...
static FAST_CODE_NOINLINE void detectAndSetCrashRecovery(const pidCrashRecovery_e crash_recovery, const int axis,
                                                         const timeUs_t currentTimeUs, const float delta,
...
static FAST_CODE_NOINLINE float applyAcroTrainer(int axis, const rollAndPitchTrims_t *angleTrim, float setPoint)
{
    ...
    if (!FLIGHT_MODE(ANGLE_MODE | HORIZON_MODE | GPS_RESCUE_MODE | ALT_HOLD_MODE | POS_HOLD_MODE)) {
        ...
        if ((pidRuntime.acroTrainerAxisState[axis] != 0) &&
            (pidRuntime.acroTrainerAxisState[axis] != setpointSign)) { // stick has reversed - stop limiting
        ...
        if (pidRuntime.acroTrainerAxisState[axis] != 0) {
            ret =
                constrainf(((pidRuntime.acroTrainerAngleLimit * angleSign) - currentAngle) * pidRuntime.acroTrainerGain,
            ...
            float checkInterval =
                constrainf(fabsf(gyro.gyroADCf[axis]) / ACRO_TRAINER_LOOKAHEAD_RATE_LIMIT, 0.0f, 1.0f) *
            ...
            if ((fabsf(projectedAngle) > pidRuntime.acroTrainerAngleLimit) && (projectedAngleSign == setpointSign)) {
                ret = ((pidRuntime.acroTrainerAngleLimit * projectedAngleSign) - projectedAngle) *
...
static float accelerationLimit(int axis, float currentPidSetpoint)
{
    ...
    if (fabsf(currentVelocity) > pidRuntime.maxVelocity[axis]) {
        currentPidSetpoint = (currentVelocity > 0) ? previousSetpoint[axis] + pidRuntime.maxVelocity[axis]
...
#if defined(USE_ABSOLUTE_CONTROL)
STATIC_UNIT_TESTED void applyAbsoluteControl(const int axis, const float gyroRate, float *currentPidSetpoint,
                                             float *itermErrorRate)
{
    if (pidRuntime.acGain > 0 || debugMode == DEBUG_AC_ERROR) {
        ...
        if (wasThrottleRaised()) {
            axisError[axis] = constrainf(axisError[axis] + acErrorRate * pidRuntime.dT, -pidRuntime.acErrorLimit,
                                         pidRuntime.acErrorLimit);
            const float acCorrection =
                constrainf(axisError[axis] * pidRuntime.acGain, -pidRuntime.acLimit, pidRuntime.acLimit);
...
STATIC_UNIT_TESTED void applyItermRelax(const int axis, const float iterm, const float gyroRate, float *itermErrorRate,
...
static FAST_CODE_NOINLINE void disarmOnImpact(void)
{
    ...
    if (wasThrottleRaised()
        ...
        if ((acc.jerkMagnitude * lowAltitudeSensitivity) > pidRuntime.landingDisarmThreshold) {
            ...
            // note: threshold should be high enough to avoid unwanted disarms in the air on throttle chops, eg around
            // 10
        }
    }
...
{
#ifdef USE_WING
    float sTerm = setpoint / getMaxRcRate(axis) * 1000.0f * (float)pidProfile->pid[axis].S * S_TERM_SCALE;
...
NOINLINE static void calculateSpaValues(const pidProfile_t *pidProfile)
{
    ...
    for (int axis = 0; axis < XYZ_AXIS_COUNT; axis++) {
        ...
        pidRuntime.spa[axis] = 1.0f - smoothStepUpTransition(fabsf(currentRate), pidProfile->spa_center[axis],
...
// Betaflight pid controller, which will be maintained in the future with additional features specialised for current
// (mini) multirotor usage. Based on 2DOF reference design (matlab)
void FAST_CODE pidController(const pidProfile_t *pidProfile, timeUs_t currentTimeUs)
{
    ...
    DEBUG_SET(DEBUG_ANTI_GRAVITY, 2,
              lrintf((1 + (pidRuntime.itermAccelerator / pidRuntime.pidCoefficient[FD_PITCH].Ki)) * 1000));
    ...
    for (int axis = FD_ROLL; axis <= FD_YAW; ++axis) {
        ...
        if (debugMode == DEBUG_D_LPF && axis != FD_YAW) {
            const float delta =
                (previousRawGyroRateDterm[axis] - gyroRateDterm[axis]) * pidRuntime.pidFrequency / D_LPF_RAW_SCALE;
        ...
        gyroRateDterm[axis] =
            pidRuntime.dtermNotchApplyFn((filter_t *)&pidRuntime.dtermNotch[axis], gyroRateDterm[axis]);
        gyroRateDterm[axis] =
            pidRuntime.dtermLowpassApplyFn((filter_t *)&pidRuntime.dtermLowpass[axis], gyroRateDterm[axis]);
        gyroRateDterm[axis] =
            pidRuntime.dtermLowpass2ApplyFn((filter_t *)&pidRuntime.dtermLowpass2[axis], gyroRateDterm[axis]);
    }
    ...
    for (flight_dynamics_index_t axis = FD_ROLL; axis <= FD_YAW; ++axis) {
        ...
        if (axis < FD_YAW) {
            ...
            if (levelMode == LEVEL_MODE_RP) {
                ...
                if (earthRefGain) {
                    ...
                    float maxAngleTargetAbs = earthRefGain * fmaxf(fabsf(pidRuntime.angleTarget[FD_ROLL]),
...
#if defined(USE_ACC)
        handleCrashRecovery(pidProfile->crash_recovery, angleTrim, axis, currentTimeUs, gyroRate, &currentPidSetpoint,
                            &errorRate);
#endif
        ...
        if (axis == FD_YAW) {
            pidData[axis].P =
                pidRuntime.ptermYawLowpassApplyFn((filter_t *)&pidRuntime.ptermYawLowpass, pidData[axis].P);
        }
        ...
        if (FLIGHT_MODE(ANGLE_MODE) && pidRuntime.axisInAngleMode[axis]) {
            ...
            // if uncommented, and the forcing to zero is removed, the two following lines will restore PID feedforward
        ...
        pidRuntime.previousPidSetpoint[axis] =
            currentPidSetpoint; // this is the value sent to blackbox, and used for D-max setpoint
            ...
            if (pidRuntime.dMaxPercent[axis] > 1.0f) {
                ...
                if (debugMode == DEBUG_D_MAX && axis == gyro.gyroDebugAxis) {
                    ...
                    DEBUG_SET(DEBUG_D_MAX, 2,
                              lrintf(pidRuntime.pidCoefficient[axis].Kd * dMaxMultiplier * 10 /
                                     DTERM_SCALE)); // effective Kd after Dmax boost
        ...
        if (launchControlActive) {
            ...
            const int launchControlYawItermLimit =
                (pidRuntime.launchControlMode == LAUNCH_CONTROL_MODE_FULL) ? LAUNCH_CONTROL_YAW_ITERM_LIMIT : 0;
        ...
        if (axis != FD_YAW) {
            ...
            const float antiGravityPBoost =
                1.0f + (pidRuntime.antiGravityThrottleD / agSetpointAttenuator) * pidRuntime.antiGravityPGain;
        ...
        if (axis == FD_YAW && pidRuntime.useIntegratedYaw) {
            ...
            pidData[axis].Sum -=
                pidData[axis].Sum * pidRuntime.integratedYawRelax / 100000.0f * pidRuntime.dT / 0.000125f;
        } else
#endif
        {
...
void dynLpfDTermUpdate(float throttle)
{
    if (pidRuntime.dynLpfFilter != DYN_LPF_NONE) {
        ...
        if (pidRuntime.dynLpfCurveExpo > 0) {
            cutoffFreq =
                dynLpfCutoffFreq(throttle, pidRuntime.dynLpfMin, pidRuntime.dynLpfMax, pidRuntime.dynLpfCurveExpo);
        } else {
