...
static void pidBetaflight(const pidProfile_t *pidProfile, uint16_t max_angle_inclination,
         const rollAndPitchTrims_t *angleTrim, const rxConfig_t *rxConfig)
{
    float errorRate = 0, rD = 0, PVRate = 0, dynKp[3];
    float ITerm,PTerm,DTerm;
    static float lastRateError[2];
    static float Kp[3], Ki[3], Kd[3], c[3], rollPitchMaxVelocity, yawMaxVelocity, previousSetpoint[3], KpDynFactor[3];
    float delta;
    int axis;
    ...
    for (axis = 0; axis < 3; axis++) {
        ...
        if ((pidProfile->P8[axis] != configP[axis]) || (pidProfile->I8[axis] != configI[axis]) || (pidProfile->D8[axis] != configD[axis])) {
            ...
            KpDynFactor[axis] = Kp[axis] * (pidProfile->ptermAccelRatio / 100.0f);
        ...
        dynKp[axis] = (KpDynFactor[axis]) ? Kp[axis] + (KpDynFactor[axis] * rcInput[axis]) : Kp[axis];
        PTerm = dynKp[axis] * errorRate * tpaFactor;
