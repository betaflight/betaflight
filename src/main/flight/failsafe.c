...
#ifdef USE_RACE_PRO
#define DEFAULT_FAILSAFE_RECOVERY_DELAY                                                                                \
    1            // 100ms of valid rx data needed to allow recovery from failsafe and arming block
#else
#define DEFAULT_FAILSAFE_RECOVERY_DELAY                                                                                \
    5            // 500ms of valid rx data needed to allow recovery from failsafe and arming block
#endif
...
bool failsafeIsReceivingRxData(void)
{
    ...
    // False with BOXFAILSAFE switch or when no valid packets for RXLOSS_TRIGGER_INTERVAL or any flight channel invalid
    // for 300ms, becomes true immediately BOXFAILSAFE switch reverts, or after recovery period expires when valid
    // packets are received rxLinkState RXLINK_DOWN (not up) is the trigger for the various failsafe stage 2 outcomes.
}
...
// rxLinkState will go RXLINK_UP immediately if BOXFAILSAFE goes back ON since receivingRxDataPeriodPreset is set to
// zero in that case otherwise RXLINK_UP is delayed for the recovery period (failsafe_recovery_delay, default 500ms,
// 1-20, min 0.1s)
{
    ...
    if (cmp32(failsafeState.validRxDataReceivedAt, failsafeState.validRxDataFailedAt) >
        (int32_t)failsafeState.receivingRxDataPeriodPreset) {
        // receivingRxDataPeriodPreset is rxDataRecoveryPeriod unless set to zero to allow immediate control recovery
        // after switch induced failsafe rxDataRecoveryPeriod defaults to 1.0s with minimum of PERIOD_RXDATA_RECOVERY
        // (100ms) link is not considered 'up', after it has been 'down', until that recovery period has expired
        failsafeState.rxLinkState = FAILSAFE_RXLINK_UP;
        // after the rxDataRecoveryPeriod, typically 1s after receiving valid data, clear RXLOSS in OSD and permit
        // arming
...
// run from rc.c when packets are lost for more than RXLOSS_TRIGGER_INTERVAL, or immediately BOXFAILSAFE switch is
// active after the stage 1 delay has expired, sets the rxLinkState to RXLINK_DOWN, ie not up, causing
// failsafeIsReceivingRxData to become false if failsafe is configured to go direct to stage 2, this is emulated
// immediately in failsafeUpdateState()
{
    //  set RXLOSS in OSD and block arming after RXLOSS_TRIGGER_INTERVAL of frame loss
    setArmingDisabled(ARMING_DISABLED_RX_FAILSAFE);

    failsafeState.validRxDataFailedAt = millis();
    if ((cmp32(failsafeState.validRxDataFailedAt, failsafeState.validRxDataReceivedAt) >
         (int32_t)failsafeState.rxDataFailurePeriod)) {
...
// triggered directly, and ONLY, by the scheduler, at 10ms = PERIOD_RXDATA_FAILURE - intervals
{
    ...
    if (IS_RC_MODE_ACTIVE(BOXFAILSAFE) && (failsafeConfig()->failsafe_switch_mode == FAILSAFE_SWITCH_MODE_STAGE2)) {
        // Force immediate stage 2 responses if mode is failsafe stage2 to emulate immediate loss of signal without
        // waiting
            ...
            if (armed) {
                ...
                if (calculateThrottleStatus() != THROTTLE_LOW) {
                    failsafeState.throttleLowPeriod =
                        millis() + failsafeConfig()->failsafe_throttle_low_delay * MILLIS_PER_TENTH_SECOND;
                }
            ...
            if (receivingRxData) {
                ...
                case FAILSAFE_PROCEDURE_AUTO_LANDING:
                    ...
                    failsafeState.landingShouldBeFinishedAt =
                        millis() + failsafeConfig()->failsafe_landing_time * MILLIS_PER_SECOND;
                    break;
        ...
        case FAILSAFE_GPS_RESCUE:
            if (receivingRxData) {
                if (areSticksActive(failsafeConfig()->failsafe_stick_threshold) ||
                    failsafeState.boxFailsafeSwitchWasOn) {
