...
static void calculateThrottleAndCurrentMotorEndpoints(timeUs_t currentTimeUs)
{
    ...
    if (mixerRuntime.feature3dEnabled) {
        ...
        if (mixerRuntime.dynIdleMinRps > 0.0f) {
            const float maxIncrease = wasThrottleRaised() ? mixerRuntime.dynIdleMaxIncrease : mixerRuntime.dynIdleStartIncrease;
        ...
        motorRangeMax = isCrashFlipModeActive() ? mixerRuntime.motorOutputHigh
                                                : mixerRuntime.motorOutputHigh - motorRangeAttenuationFactor * (mixerRuntime.motorOutputHigh - mixerRuntime.motorOutputLow);
#else
        motorRangeMax = mixerRuntime.motorOutputHigh;
#endif
...
static bool applyCrashFlipModeToMotors(void)
{
    ...
    for (int i = 0; i < mixerRuntime.motorCount; ++i) {
        float motorOutputNormalised = signPitch * mixerRuntime.currentMixer[i].pitch + signRoll * mixerRuntime.currentMixer[i].roll + signYaw * mixerRuntime.currentMixer[i].yaw;
...
static void applyRpmLimiter(mixerRuntime_t *mixer)
{
    ...
    } else if (pidOutput < -400.0f * pidGetDT() && lrintf(rcCommand[THROTTLE]) >= rxConfig()->maxcheck - STICK_HIGH_DEADBAND &&
    ...
    float rpmLimiterThrottleScaleOffset = pt1FilterApply(&mixer->rpmLimiterThrottleScaleOffsetFilter,
                                                         constrainf(mixer->rpmLimiterRpmLimit / motorEstimateMaxRpm(), 0.0f, 1.0f) - mixer->rpmLimiterInitialThrottleScale);
...
FAST_CODE_NOINLINE void mixTable(timeUs_t currentTimeUs)
{
    ...
    if (featureIsEnabled(FEATURE_MOTOR_STOP) && ARMING_FLAG(ARMED) && !mixerRuntime.feature3dEnabled && !airmodeEnabled &&
