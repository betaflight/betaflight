...
static void calculateThrottleAndCurrentMotorEndpoints(timeUs_t currentTimeUs)
{
    ...
    if (mixerRuntime.feature3dEnabled) {
        ...
        if (mixerRuntime.dynIdleMinRps > 0.0f) {
            ...
            DEBUG_SET(DEBUG_DYN_IDLE, 3, lrintf(minRps * 10.0f));  //!< Minimum Motor Frequency [0.1Hz]
            ...
            DEBUG_SET(DEBUG_DYN_IDLE, 0, MAX(-1000, lrintf(dynIdleP * 10000)));   //!< Dyn Idle P [0.0001]
            DEBUG_SET(DEBUG_DYN_IDLE, 1, lrintf(mixerRuntime.dynIdleI * 10000));  //!< Dyn Idle I [0.0001]
            DEBUG_SET(DEBUG_DYN_IDLE, 2, lrintf(dynIdleD * 10000));               //!< Dyn Idle D [0.0001]
        } else {
        ...
        if (mixerRuntime.vbatSagCompensationFactor > 0.0f) {
            ...
            DEBUG_SET(DEBUG_BATTERY, 2, lrintf(batteryGoodness * 100));               //!< Sag Compensation Battery Goodness [%]
            DEBUG_SET(DEBUG_BATTERY, 3, lrintf(motorRangeAttenuationFactor * 1000));  //!< Sag Compensation Attenuation [0.001]
        }
...
static void applyRpmLimiter(mixerRuntime_t *mixer)
{
    ...
    DEBUG_SET(DEBUG_RPM_LIMIT, 0, lrintf(averageRpm));                               //!< Average RPM [rpm]
    DEBUG_SET(DEBUG_RPM_LIMIT, 1, lrintf(rpmLimiterThrottleScaleOffset * 100.0f));   //!< Throttle Scale Offset [%]
    DEBUG_SET(DEBUG_RPM_LIMIT, 2, lrintf(mixer->rpmLimiterThrottleScale * 100.0f));  //!< Throttle Scale [%]
    DEBUG_SET(DEBUG_RPM_LIMIT, 3, lrintf(throttle * 100.0f));                        //!< Limited Throttle [%]
    DEBUG_SET(DEBUG_RPM_LIMIT, 4, lrintf(error));                                    //!< RPM Error [rpm]
    DEBUG_SET(DEBUG_RPM_LIMIT, 5, lrintf(p * 100.0f));                               //!< P Term [%]
    DEBUG_SET(DEBUG_RPM_LIMIT, 6, lrintf(mixer->rpmLimiterI * 100.0f));              //!< I Term [%]
    DEBUG_SET(DEBUG_RPM_LIMIT, 7, lrintf(d * 100.0f));                               //!< D Term [%]
}
#endif // USE_RPM_LIMIT
...
static void applyMixToMotors(const float motorMix[MAX_SUPPORTED_MOTORS], motorMixer_t *activeMixer)
{
    ...
    DEBUG_SET(DEBUG_EZLANDING, 1, throttle * 10000U);  //!< Adjusted Throttle [0.0001]
...
static float calcEzLandLimit(float maxDeflection, float speed)
{
    ...
    DEBUG_SET(DEBUG_EZLANDING, 4, lrintf(deflectionLimit * 10000.0f));  //!< Stick Deflection Limit [0.0001]
    ...
    DEBUG_SET(DEBUG_EZLANDING, 5, lrintf(speedLimit * 10000.0f));  //!< Speed Limit [0.0001]
...
static void applyMixerAdjustmentEzLand(float *motorMix, const float motorMixMin, const float motorMixMax)
{
    ...
    DEBUG_SET(DEBUG_EZLANDING, 0, fminf(1.0f, ezLandFactor) * 10000U);  //!< EZ Land Factor [0.0001]
    // DEBUG_EZLANDING 1 is the adjusted throttle
    DEBUG_SET(DEBUG_EZLANDING, 2, upperLimit * 10000U);                                 //!< Upper Throttle Limit [0.0001]
    DEBUG_SET(DEBUG_EZLANDING, 3, fminf(1.0f, ezLandLimit / absMotorMixMin) * 10000U);  //!< EZ Land Limit [0.0001]
