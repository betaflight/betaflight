...
void calculateThrottleAndCurrentMotorEndpoints(void)
{
    ...
    if(feature(FEATURE_3D)) {
        ...
        if(rcCommand[THROTTLE] <= rcCommand3dDeadBandLow) {
            ...
            bool yaw_motors_reversed_coll_pitch = true // Yaw is reversed when inverted for a collective pitch quad
            ...
            motorOutputMixSign = 1;
            bool yaw_motors_reversed_coll_pitch = false
            rcThrottlePrevious = rcCommand[THROTTLE];
...
void mixTable(uint8_t vbatPidCompensation)
{
    ...
    if (yaw_motors_reversed_coll_pitch) {
    scaledAxisPidYaw = -scaledAxisPidYaw;
    }
