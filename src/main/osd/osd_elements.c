...
static void osdElementAATTelemetry(osdElementParms_t *element)
{
    ...
    fmtbuf[30] = 0;
    osdDisplayWrite(element, 0,0, DISPLAYPORT_ATTR_NONE, fmtbuf);

    if (osdConfig()->osd_telemetry > 1){
        osdDisplayWrite(element, 0, 3, DISPLAYPORT_ATTR_NONE, fmtbuf);   // Test display because normal telemetry line is not visible
    }
}
