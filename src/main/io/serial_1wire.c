...
static bool  IsUsb1WireActivated = false;
...
void usb1WirePassthrough(uint8_t escIndex)
{
    ...
    __disable_irq();
    if (!IsUsb1WireActivated) {
        // only once needed: set all ESC inputs to input with pull up
        for (volatile uint8_t i = 0; i < escCount; i++) {
             gpio_set_mode(escHardware[i].gpio,escHardware[i].pin, Mode_IPU); //GPIO_Mode_IPU
        }
        IsUsb1WireActivated = true;
    }
