...
{
    DEBUG_SET(DEBUG_FLASH_TEST_PRBS, 0, 1);  //!< Test State
...
void checkFlashStop(void)
{
    ...
    DEBUG_SET(DEBUG_FLASH_TEST_PRBS, 7, checkFlashErrors);  //!< Error Count
...
void flashfsFlushSync(void)
{
    ...
    while (!flashfsBufferIsEmpty()) {
        ...
        // bytes that were just drained.
        //
        // This is the interlock #15525 added to the single-shot form of this
        // function, applied once per iteration, since this loop re-captures the
        // descriptors each time round and it is the capture that goes stale.
