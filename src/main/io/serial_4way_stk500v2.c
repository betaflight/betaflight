...
static uint8_t _CMD_LOAD_ADDRESS(void)
{
	...
	StkSendByte(0);
	StkSendByte(0);
	StkSendByte(wtf.D_FLASH_ADDR_H);
	StkSendByte(wtf.D_FLASH_ADDR_L);
	StkSendPacketFooter();
...
static uint8_t _CMD_READ_MEM_ISP()
{
	...
	StkSendByte(StkCmd);
	StkSendByte(LenHi);
	StkSendByte(wtf.D_NUM_BYTES);
	StkSendByte(CmdFlashEepromRead);
	StkSendPacketFooter();
	return (StkRcvPacket(wtf.D_PTR_I));
}

static uint8_t _CMD_PROGRAM_MEM_ISP(void)
{
    union uint8_16u Len;
    uint8_t LenLo=wtf.D_NUM_BYTES;
    uint8_t LenHi;
    if (LenLo) {
        LenHi=0;
        Len.word=LenLo+10;
