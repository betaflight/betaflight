...
void pidAudioUpdate(void)
{
    ...
    case PID_AUDIO_PIDSUM_X:
        {
            const uint32_t pidSumX = MIN(ABS(pidData[FD_ROLL].PIDSum), PIDSUM_LIMIT);
            tone = scaleRange(pidSumX, 0, PIDSUM_LIMIT, TONE_MAX, TONE_MIN);
            break;
        }
    case PID_AUDIO_PIDSUM_Y:
        {
            const uint32_t pidSumY = MIN(ABS(pidData[FD_PITCH].PIDSum), PIDSUM_LIMIT);
            tone = scaleRange(pidSumY, 0, PIDSUM_LIMIT, TONE_MAX, TONE_MIN);
            break;
        }
    case PID_AUDIO_PIDSUM_XY:
        {
            const uint32_t pidSumXY = MIN((ABS(pidData[FD_ROLL].PIDSum) + ABS(pidData[FD_PITCH].PIDSum)) / 2, PIDSUM_LIMIT);
