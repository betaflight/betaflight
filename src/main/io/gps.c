...
static void gpsWeekTimeToDateTime(gpsDateTime_t *dt, int16_t week, uint32_t timeOfWeekMs, int32_t timeOfWeekNs)
{
    ...
    // Convert to Unix time and adjust for leap seconds to get UTC
    ...
    *dashboardGpsPacketLogCurrentChar = DASHBOARD_LOG_IGNORED;
    ...
    case CLSMSG(CLASS_NAV, MSG_NAV_PVT):
        ...
        *dashboardGpsPacketLogCurrentChar = DASHBOARD_LOG_UBLOX_SOL;
        ...
        ubxHaveNewSpeed = true;
        // Store GPS date/time for telemetry
        gpsSol.dateTime.valid = (ubxRcvMsgPayload.ubxNavPvt.valid & NAV_VALID_DATE) && (ubxRcvMsgPayload.ubxNavPvt.valid & NAV_VALID_TIME);
        if (gpsSol.dateTime.valid) {
            gpsSol.dateTime.year = ubxRcvMsgPayload.ubxNavPvt.year;
            gpsSol.dateTime.month = ubxRcvMsgPayload.ubxNavPvt.month;
            gpsSol.dateTime.day = ubxRcvMsgPayload.ubxNavPvt.day;
            gpsSol.dateTime.hour = ubxRcvMsgPayload.ubxNavPvt.hour;
            gpsSol.dateTime.min = ubxRcvMsgPayload.ubxNavPvt.min;
            gpsSol.dateTime.sec = ubxRcvMsgPayload.ubxNavPvt.sec;
            gpsSol.dateTime.millis = (ubxRcvMsgPayload.ubxNavPvt.nano > 0) ? ubxRcvMsgPayload.ubxNavPvt.nano / 1000000 : 0;
        }
#ifdef USE_RTC_TIME
        // Set system clock once when GPS time is available
        if (!rtcHasTime() && gpsSol.dateTime.valid) {
