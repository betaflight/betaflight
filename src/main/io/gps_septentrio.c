...
static bool sbfCommitNavEpoch(void)
{
	...
	gpsSol.llh.lat = (int32_t)lround(RADIANS_TO_DEGREES((float)pvt->latitude) * GPS_DEGREES_DIVIDER);
...
static void sbfProcessChannelStatus(void)
{
    ...
    for (unsigned i = 0; i < GPS_numCh; i++) { // loop over the number of satellites reported by the receiver 
		...
		if (gnssId == SEPTENTRIO_GNSS_UNKNOWN || maxTrack == 0) { // unknown constellation or idle/not applicable antenna tracking status
            sat += header->sb1Length + s1.n2 * header->sb2Length; // skip to the next ChannelSatInfo sub-block
            continue; 
        }
        ...
        sat += header->sb1Length + s1.n2 * header->sb2Length; // advance to the next ChannelSatInfo sub-block
	}
