...
static int32_t currentMeterADCToCentiamps(const uint16_t src)
{
    ...
    DEBUG_SET(DEBUG_CURRENT_SENSOR, 0, millivolts);  //!< ADC Voltage [unit:0.001V]
    DEBUG_SET(DEBUG_CURRENT_SENSOR, 1, centiAmps);   //!< Current [unit:0.01A]
...
void currentMeterADCRead(currentMeter_t *meter)
{
    ...
    DEBUG_SET(DEBUG_CURRENT_SENSOR, 2, meter->amperageLatest);  //!< Latest Amperage [unit:0.01A]
    DEBUG_SET(DEBUG_CURRENT_SENSOR, 3, meter->mAhDrawn);        //!< Capacity Drawn [unit:mAh]
}
