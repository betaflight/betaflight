...
void dynLpfGyroUpdate(float throttle)
{
    if (gyro.dynLpfFilter != DYN_LPF_NONE) {
        ...
        if (gyro.dynLpfThrPercent) {
            const float throttlePercent = (float)gyro.dynLpfThrPercent / 100.0f;
            const float minWeight = (float)gyro.dynLpfMinWeight / 100.0f;
        ...
        case DYN_LPF_PT1:
            for (int axis = 0; axis < XYZ_AXIS_COUNT; axis++) {
                pt1FilterUpdateCutoffWeighted(&gyro.lowpassFilter[axis].pt1FilterState, pt1FilterGain(cutoffFreq, gyroDt), weight);
            }
            break;
        case DYN_LPF_BIQUAD:
            for (int axis = 0; axis < XYZ_AXIS_COUNT; axis++) {
