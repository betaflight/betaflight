...
void rangefinderUPT1Update(rangefinderDev_t *dev)
{
    ...
    while (serialRxBytesWaiting(upt1SerialPort)) {
        ...
        DEBUG_SET(DEBUG_UPT1, 7, upt1FrameState);  //!< Frame Parser State [enum:upt1FrameState_e]
        ...
        case UPT1_FRAME_STATE_WAIT_FOOTER:
            if (c == UPT1_FRAME_FOOTER) {
                ...
                DEBUG_SET(DEBUG_UPT1, 0, distanceMm);        //!< Distance [unit:0.001m]
                DEBUG_SET(DEBUG_UPT1, 1, confidence);        //!< Confidence [unit:%]
                DEBUG_SET(DEBUG_UPT1, 2, (int16_t)flow_x);   //!< Optical Flow X
                DEBUG_SET(DEBUG_UPT1, 3, (int16_t)flow_y);   //!< Optical Flow Y
                DEBUG_SET(DEBUG_UPT1, 4, validStatus);       //!< Laser Valid Status
                DEBUG_SET(DEBUG_UPT1, 5, integration_time);  //!< Integration Time [unit:us]
                ...
                if (laserValid && distanceInRange) {
                    ...
                    DEBUG_SET(DEBUG_UPT1, 6, upt1Value);  //!< Reported Distance [unit:cm]
                } else {
                    upt1Value = RANGEFINDER_OUT_OF_RANGE;
                    // -99 marks the sample as out of range
                    DEBUG_SET(DEBUG_UPT1, 6, -99);  //!< Reported Distance [unit:cm]
                }
