        ...
        case UPT1_FRAME_STATE_WAIT_FOOTER:
            if (c == UPT1_FRAME_FOOTER) {
                ...
                if (validStatus == 0xF5) {  // 0xF5 indicates valid optical flow data
                    ...
                    // Convert from sensor units to angular rate (rad/s)
                    // Sensor reports flow_x/flow_y in radians*10000, integration_time in microseconds
                    ...
                    if (dt_s > 0) {
                        ...
                        // Override quality if rangefinder distance is too close
