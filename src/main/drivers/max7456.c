...
#define __spiBusTransactionBegin(busdev)                                 \
    {                                                                    \
...
#define __spiBusTransactionEnd(busdev)                                       \
    {                                                                        \
        IOHi((busdev)->busdev_u.spi.csnPin);                                 \
        spiSetDivisor((busdev)->busdev_u.spi.instance, MAX7456_RESTORE_CLK); \
    }
#else
...
void max7456ReInitIfRequired(void)
{
    ...
    if (stallCheck != videoSignalReg) {
        max7456ReInit();
    } else if ((videoSignalCfg == VIDEO_SYSTEM_AUTO) && ((nowMs - lastSigCheckMs) > MAX7456_SIGNAL_CHECK_INTERVAL_MS)) {
        ...
        if (videoSense & STAT_LOS) {
            ...
            if ((VIN_IS_PAL(videoSense) && VIDEO_MODE_IS_NTSC(videoSignalReg)) || (VIN_IS_NTSC_alt(videoSense) && VIDEO_MODE_IS_PAL(videoSignalReg))) {
