...
static int imuf9001SendReceiveCommand(const busDevice_t *bus, gyroCommands_t commandToSend, imufCommand_t *reply, imufCommand_t *data)
{
    ...
    while (failCount-- > 0)
    {
        ...
        {
            failCount -= 100;
            if (imufSendReceiveSpiBlocking(bus, (uint8_t *)&command, (uint8_t *)reply, sizeof(imufCommand_t)))
            {
...
int imuf9001Whoami(const busDevice_t *bus)
{
    imufDev = (busDevice_t *)bus;
    uint32_t attempt;
    imufCommand_t reply;

    for (attempt = 0; attempt < 5; attempt++)
    {
        if (imuf9001SendReceiveCommand(bus, IMUF_COMMAND_REPORT_INFO, &reply, NULL))
        {
...
uint8_t imuf9001SpiDetect(const busDevice_t *bus)
{
    ...
    IOInit(bus->busdev_u.spi.csnPin, OWNER_GYRO_CS, 0);
    ...
    for (int x=0; x<3; x++)
    {
        ...
        returnCheck = imuf9001Whoami(bus);
        if(returnCheck)
        {
            return returnCheck;
        }
