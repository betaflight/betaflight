...
static bool ist8310Init(magDev_t * magDev)
{
    ...
    bool ack = true;
    ack = ack && busWriteRegister(busdev, IST8310_REG_CNTRL1, IST8310_ODR_50_HZ);
    delay(5);

    ack = ack && busWriteRegister(busdev, IST8310_REG_AVERAGE, IST8310_AVG_16);
    delay(5);
...
static bool ist8310Read(magDev_t *magDev, int16_t *magData)
{
    ...
    bool ack = busReadRegisterBuffer(busdev, IST8310_REG_DATA, buf, 6);
    if (!ack) {
...
bool ist8310Detect(magDev_t *magDev)
{
    ...
    if (busdev->bustype == BUSTYPE_I2C && busdev->busdev_u.i2c.address == 0) {
        busdev->busdev_u.i2c.address = IST8310_ADDRESS;
    }

    uint8_t sig = 0;
    bool ack = busReadRegisterBuffer(busdev, IST8310_REG_WHOAMI, &sig, 1);
