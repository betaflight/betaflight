...
void pwmWriteDigital(uint8_t index, uint16_t value)
{
    ...
    value = (value << 1) | 0;                                               // Here goes telemetry bit (false for now)
    int csum = (value ^ (value >> 4) ^ (value >> 8)) & 0xf;                 // calculate checksum
    value = (value << 4) | csum;                                            // append checksum
    for(int i = 0; i < 16; i++) {
        motor->dmaBuffer[i] = (value & 0x8000) ? MOTOR_BIT_1 : MOTOR_BIT_0; // MSB first
        value <<= 1;
    }
