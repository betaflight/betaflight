...
bool i2cBusWriteRegisterBuffer(const extDevice_t *dev, uint8_t reg, uint8_t *data, uint8_t length)
{
    return i2cWriteBufferBlocking(dev->bus->busType_u.i2c.device, dev->busType_u.i2c.address, reg, length, data);
}
