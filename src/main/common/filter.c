...
FAST_CODE(cpFILTERS) float nullFilterApply(filter_t *filter, float input)
{
    UNUSED(filter);
    return input;
}
...
FAST_CODE(cpFILTERS) float pt1FilterApply(pt1Filter_t *filter, float input)
{
...
FAST_CODE(cpFILTERS) float slewFilterApply(slewFilter_t *filter, float input)
{
...
FAST_CODE(cpFILTERS) void biquadFilterUpdate(biquadFilter_t *filter, float filterFreq, uint32_t refreshRate, float Q, biquadFilterType_e filterType)
{
    // backup state
    float x1 = filter->x1;
    float x2 = filter->x2;
...
FAST_CODE(cpFILTERS) void biquadFilterUpdateLPF(biquadFilter_t *filter, float filterFreq, uint32_t refreshRate)
{
    biquadFilterUpdate(filter, filterFreq, refreshRate, BIQUAD_Q, FILTER_LPF);
}

/* Computes a biquadFilter_t filter on a sample (slightly less precise than df2 but works in dynamic mode) */
FAST_CODE(cpFILTERS) float biquadFilterApplyDF1(biquadFilter_t *filter, float input)
{
    /* compute result */
...
FAST_CODE(cpFILTERS) float biquadFilterApply(biquadFilter_t *filter, float input)
{
...
FAST_CODE(cpFILTERS) float laggedMovingAverageUpdate(laggedMovingAverage_t *filter, float input)
{
