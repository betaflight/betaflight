...
// PG_OSD_CONFIG
    ...
    { "osd_el_vbat",                     VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_MAIN_BATT_VOLTAGE]) },
    { "osd_el_rssi",                     VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_RSSI_VALUE]) },
    { "osd_el_tim_1",                    VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_ITEM_TIMER_1]) },
    { "osd_el_tim_2",                    VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_ITEM_TIMER_2]) },
    { "osd_el_remaining_time_estimate",  VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_REMAINING_TIME_ESTIMATE]) },
    { "osd_el_flymode",                  VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_FLYMODE]) },
    { "osd_el_throttle",                 VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_THROTTLE_POS]) },
    { "osd_el_vtx_channel",              VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_VTX_CHANNEL]) },
    { "osd_el_crosshairs",               VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_CROSSHAIRS]) },
    { "osd_el_ah_sbar",                  VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_HORIZON_SIDEBARS]) },
    { "osd_el_ah",                       VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_ARTIFICIAL_HORIZON]) },
    { "osd_el_current",                  VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_CURRENT_DRAW]) },
    { "osd_el_mah_drawn",                VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_MAH_DRAWN]) },
    { "osd_el_craft_name",               VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_CRAFT_NAME]) },
    { "osd_el_gps_speed",                VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_GPS_SPEED]) },
    { "osd_el_gps_lon",                  VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_GPS_LON]) },
    { "osd_el_gps_lat",                  VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_GPS_LAT]) },
    { "osd_el_gps_sats",                 VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_GPS_SATS]) },
    { "osd_el_home_dir",                 VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_HOME_DIR]) },
    { "osd_el_home_dist",                VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_HOME_DIST]) },
    { "osd_el_compass_bar",              VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_COMPASS_BAR]) },
    { "osd_el_altitude",                 VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_ALTITUDE]) },
    { "osd_el_pid_roll",                 VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_ROLL_PIDS]) },
    { "osd_el_pid_pitch",                VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_PITCH_PIDS]) },
    { "osd_el_pid_yaw",                  VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_YAW_PIDS]) },
    { "osd_el_debug",                    VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_DEBUG]) },
    { "osd_el_power",                    VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_POWER]) },
    { "osd_el_pidrate_profile",          VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_PIDRATE_PROFILE]) },
    { "osd_el_warnings",                 VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_WARNINGS]) },
    { "osd_el_avg_cell_voltage",         VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_AVG_CELL_VOLTAGE]) },
    { "osd_el_pit_ang",                  VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_PITCH_ANGLE]) },
    { "osd_el_rol_ang",                  VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_ROLL_ANGLE]) },
    { "osd_el_battery_usage",            VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_MAIN_BATT_USAGE]) },
    { "osd_el_disarmed",                 VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_DISARMED]) },
    { "osd_el_nheading",                 VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_NUMERICAL_HEADING]) },
    { "osd_el_nvario",                   VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_NUMERICAL_VARIO]) },
    { "osd_el_esc_tmp",                  VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_ESC_TMP]) },
    { "osd_el_esc_rpm",                  VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_ESC_RPM]) },
    { "osd_el_rtc_date_time",            VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_RTC_DATETIME]) },
    { "osd_el_adjustment_range",         VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_ADJUSTMENT_RANGE]) },
    { "osd_el_core_temp",                VAR_UINT8  | MASTER_VALUE | MODE_ARRAY, .config.array.length = 3, PG_OSD_CONFIG, offsetof(osdConfig_t, elements[OSD_CORE_TEMPERATURE]) },
