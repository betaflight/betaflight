    ...
    if (ARMING_FLAG(ARMED)
        ...
        if (!feature(FEATURE_MOTOR_STOP) || isAirmodeActive() || (throttleStatus != THROTTLE_LOW)) { // are motors running?
            ...
            if ((((throttlePercent >= lowThrottleLimit) && areSticksActive(RUNAWAY_TAKEOFF_DEACTIVATE_STICK_PERCENT)) || (throttlePercent >= midThrottleLimit))
                && (fabsf(pidData[FD_PITCH].PIDSum) < RUNAWAY_TAKEOFF_DEACTIVATE_PIDSUM_LIMIT)
                && (fabsf(pidData[FD_ROLL].PIDSum) < RUNAWAY_TAKEOFF_DEACTIVATE_PIDSUM_LIMIT)
                && (fabsf(pidData[FD_YAW].PIDSum) < RUNAWAY_TAKEOFF_DEACTIVATE_PIDSUM_LIMIT)) {
...
static void subTaskPidController(timeUs_t currentTimeUs)
{
    ...
    if (ARMING_FLAG(ARMED)
        ...
        if (((fabsf(pidData[FD_PITCH].PIDSum) >= RUNAWAY_TAKEOFF_PIDSUM_THRESHOLD)
            || (fabsf(pidData[FD_ROLL].PIDSum) >= RUNAWAY_TAKEOFF_PIDSUM_THRESHOLD)
            || (fabsf(pidData[FD_YAW].PIDSum) >= RUNAWAY_TAKEOFF_PIDSUM_THRESHOLD))
