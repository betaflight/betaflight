...
static int applyStepAdjustment(controlRateConfig_t *controlRateConfig, uint8_t adjustmentFunction, int delta)
{
    ...
    case ADJUSTMENT_PITCH_D:
        newValue = constrain((int)currentPidProfile->d_max[FD_PITCH] + delta, 0, 200); // FIXME magic numbers repeated in cli.c
    ...
    case ADJUSTMENT_ROLL_D:
        newValue = constrain((int)currentPidProfile->d_max[FD_ROLL] + delta, 0, 200); // FIXME magic numbers repeated in cli.c
