    ...
    case ADJUSTMENT_SIMPLIFIED_GYRO_MULTIPLIER:
        {
                ...
                // Unit tests don't link the full simplified tuning / gyro config stack.
                // Avoid pulling in gyroConfigMutable and PG symbols.
                int stubValue = 100; // Provide a safe stub value for unit tests
                int result = applyCenterRelativeAdjustment(
...
static void applyGyroTuning(void)
{
    #if defined(UNIT_TEST)
        // Unit tests don't link the full simplified tuning / gyro config stack.
        // Avoid pulling in applySimplifiedTuningGyroFilters and PG symbols.
        return;
    #else
