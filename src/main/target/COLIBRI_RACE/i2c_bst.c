...
static bool bstSlaveProcessFeedbackCommand(uint8_t bstRequest)
{
    ...
    switch (bstRequest) {
        ...
        case BST_STATUS:
            ...
            bstWrite16(0);
#endif
            bstWrite16(sensors(SENSOR_ACC) | sensors(SENSOR_BARO) << 1 | sensors(SENSOR_MAG) << 2 | sensors(SENSOR_GPS) << 3 | sensors(SENSOR_ALTIMETER) << 4);
            ...
            tmp = IS_ENABLED(FLIGHT_MODE(ANGLE_MODE)) << BOXANGLE |
                    ...
                    IS_ENABLED(FLIGHT_MODE(ALTIMETER_MODE)) << BOXALTIMETER |
...
static bool bstSlaveUSBCommandFeedback(/*uint8_t bstFeedback*/)
{
    ...
    bstWrite8(sensors(SENSOR_ACC) | sensors(SENSOR_BARO) << 1 | sensors(SENSOR_MAG) << 2 | sensors(SENSOR_GPS) << 3 | sensors(SENSOR_ALTIMETER) << 4);
    bstWrite8(0x00);
    bstWrite8(0x00);
    bstWrite8(0x00);
...
bool writeFCModeToBST(void)
{
    ...
    tmp = IS_ENABLED(ARMING_FLAG(ARMED)) |
           ...
           IS_ENABLED(FLIGHT_MODE(ALTIMETER_MODE)) << 6 |
           IS_ENABLED(FLIGHT_MODE(FAILSAFE_MODE)) << 7;

    bstMasterStartBuffer(PUBLIC_ADDRESS);
    bstMasterWrite8(CLEANFLIGHT_MODE_FRAME_ID);
    bstMasterWrite8(tmp);
    bstMasterWrite8(sensors(SENSOR_ACC) | sensors(SENSOR_BARO) << 1 | sensors(SENSOR_MAG) << 2 | sensors(SENSOR_GPS) << 3 | sensors(SENSOR_ALTIMETER) << 4);
