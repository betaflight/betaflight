...
STATIC_UNIT_TESTED uint8_t crsfFrameCRC(const rxRuntimeState_t  *const rxRuntimeState)
{ 
...
STATIC_UNIT_TESTED uint8_t crsfFrameCmdCRC(const rxRuntimeState_t *const rxRuntimeState)
{
    const crsfFrame_t *const crsfFrame = &rxRuntimeState->incomingFrame->crsf;
    // CRC includes type and payload
        ...
        if (crsfFramePosition >= fullFrameLength) {
            ...
            if (crc == crsfFrame->bytes[fullFrameLength - 1]) {
                ...
                case CRSF_FRAMETYPE_COMMAND:
                    if ((crsfFrame->bytes[fullFrameLength - 2] == crsfFrameCmdCRC(rxRuntimeState)) &&
...
STATIC_UNIT_TESTED uint8_t crsfFrameStatus(rxRuntimeState_t *rxRuntimeState)
{
    ...
    if (crsfFrameDone) {
        crsfFrameDone = false;
        const crsfFrame_t *const crsfChannelDataFrame = &rxRuntimeState->validatedFrame->crsf;
...
void crsfRxSendTelemetryData()
{
    ...
    if (telemetryBufLen > 0) {
        serialWriteBuf(rxGetSerialPort(), telemetryBuf, telemetryBufLen);
...
{
    serialSetBaudRate(rxGetSerialPort(), baudrate);
}
#endif
