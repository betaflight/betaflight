...
static void buildTelemetryFrame(const uint8_t *packet)
{
    ...
    case FRSKY_SPI_A1_SOURCE_EXTADC:
        a1Value = (adcGetValue(ADC_EXTERNAL1) & 0xff0) >> 4;
        break;
#if defined(USE_TELEMETRY_FRSKY_HUB)
    ...
    const uint8_t a2Value = (adcGetValue(ADC_RSSI)) >> 4;
...
void frSkyDSetRcData(uint16_t *rcData, const uint8_t *packet)
{
    ...
    if (!dataError) {
        ...
        DEBUG_SET(DEBUG_RX_FRSKY_SPI, DEBUG_DATA_ERROR_COUNT, ++dataErrorCount);  //!< Data Error Count
    }
}
