...
rx_spi_received_e frSkyXHandlePacket(uint8_t * const packet, uint8_t * const protocolState)
{
    ...
    case STATE_DATA:
        if (rxSpiGetExtiState() && (!frameReceived)) {
            ...
            if (ccLen >= packetLength) {
                ...
                if (!frameReceived) {
                    packetErrors++;
                    DEBUG_SET(DEBUG_RX_FRSKY_SPI, DEBUG_DATA_BAD_FRAME, packetErrors);  //!< Packet Errors
    ...
    case STATE_RESUME:
        if (cmpTimeUs(micros(), packetTimerUs) > receiveDelayUs + 3700) {
            ...
            missingPackets++;
            DEBUG_SET(DEBUG_RX_FRSKY_SPI, DEBUG_DATA_MISSING_PACKETS, missingPackets);  //!< Missing Packets
            *protocolState = STATE_DATA;
        }
        break;
    }
