...
BOOT_CODE static void relocate_runtime_image(void)
{
    ...
    extern uint32_t __tcm_vector_copy_src_start__;
    extern uint32_t __tcm_vector_copy_dst_start__;
    extern uint32_t __tcm_vector_copy_end__;

    volatile uint32_t *special_src = &__tcm_vector_copy_src_start__;
    volatile uint32_t *special_dst = &__tcm_vector_copy_dst_start__;
    uint32_t *special_end = &__tcm_vector_copy_end__;

    if (special_dst != special_src)
    {
        boot_copy_words(special_dst, special_src, special_end);
    }
}
...
BOOT_CODE void _main(void)
{
    ...
    boot_copy_words((volatile uint32_t *)&_sdmaram_data,
                    (volatile const uint32_t *)&_sdmaram_idata,
                    (volatile uint32_t *)&_edmaram_data);
    boot_zero_words((volatile uint32_t *)&_sdmaram_bss,
                    (volatile uint32_t *)&_edmaram_bss);
    boot_copy_words((volatile uint32_t *)&_sfastram_data,
                    (volatile const uint32_t *)&_sfastram_idata,
                    (volatile uint32_t *)&_efastram_data);
    boot_zero_words((volatile uint32_t *)&_sfastram_bss,
                    (volatile uint32_t *)&_efastram_bss);
    boot_copy_words(&__ram_code_start__,
                    &__ram_code_load_start__,
                    &__ram_code_end__);
    ...
    /* Switch to the application runtime stack from the startup vector table. */
