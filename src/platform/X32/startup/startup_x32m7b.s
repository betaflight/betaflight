/*
 * X32M7xx XIP-to-TCM Startup (GCC, Cortex-M7 Thumb-2)
 *
...
/* BRIDGE_HANDLER from_name, to_name
 *
 * Provides a strong symbol for <from_name> (overriding the WEAK_HANDLER
 * fallback) that tail-calls <to_name>.  Also provides a weak fallback for
 * <to_name> so the linker never produces an undefined-reference error when
 * the target handler is conditionally compiled out.
 */
.macro BRIDGE_HANDLER from_name, to_name
    .section .text.\from_name, "ax", %progbits
    .global \from_name
    .type   \from_name, %function
    .thumb_func
\from_name:
    b       \to_name
    .size   \from_name, . - \from_name
    .section .text.\to_name, "ax", %progbits
    .weak   \to_name
    .type   \to_name, %function
    .thumb_func
\to_name:
    b       Default_Handler
    .size   \to_name, . - \to_name
.endm
  ...
  BRIDGE_HANDLER  USART4_IRQHandler, UART4_IRQHandler
  BRIDGE_HANDLER  USART5_IRQHandler, UART5_IRQHandler
  WEAK_HANDLER  USART6_IRQHandler
  BRIDGE_HANDLER  USART7_IRQHandler, UART7_IRQHandler
  BRIDGE_HANDLER  USART8_IRQHandler, UART8_IRQHandler
  WEAK_HANDLER  UART9_IRQHandler
  BRIDGE_HANDLER  UART10_IRQHandler, USART10_IRQHandler
