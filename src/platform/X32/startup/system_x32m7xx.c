...
#define X32_ROM_READ_TCM_SIZE    (0x1ff00f01U)
#define X32_OTP_TCM_SIZE_REG     (0x51105280U)
/**
 *\*\name   ConfigTcmSize.
 *\*\fun    Config TCM_SIZE
 *\*\param  tcmSizeValue 
...
{
    uint32_t requestValue = tcmSizeValue & OTPC_CRLD2_TCMSZ;
    uint32_t currValue = (*(uint32_t(*)(void))X32_ROM_READ_TCM_SIZE)();

    /* Match the vendor boot flow: only program TCM size from the blank OTP state. */
    if((requestValue <= 0x2f) && (currValue == 0x2c) && (currValue != requestValue))
    {
        *(__IO uint32_t*)X32_OTP_TCM_SIZE_REG = requestValue;
        __Self_SystemReset();
    }
}
#ifdef CORE_CM7
