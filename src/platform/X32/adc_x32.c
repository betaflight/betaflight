...
static volatile DMA_DATA_ZERO_INIT uint16_t adcConversionBuffer[ADC_SOURCE_COUNT];
...
static void setScalingFactors(void)
{
    ...
    /* Read the data at addr */
    OTPC_ReadWord(0x03B0, &value);
    VTS_value = (uint16_t)(value&0xFFFF);
    if(VTS_value != 0xFFFF)
    {
        adcTSCAL1 = (VTS_value * 4095.0f) / 3300.0f;
    }
    else
    {
        adcTSCAL1 = (TEMPSENSOR_CAL1_V * 4095.0f) / 3.3f;
...
void adcInit(const adcConfig_t *config)
{
    ...
    for (int i = 0; i < ADC_SOURCE_COUNT; i++) {
        ...
        if (adcOperatingConfig[i].sampleTime == 0) {
        ...
        if (adcOperatingConfig[i].tag) {
            ...
            IOConfigGPIO(IOGetByTag(adcOperatingConfig[i].tag), IO_CONFIG(GPIO_MODE_ANALOG, GPIO_SPEED_FREQ_LOW, GPIO_NOPULL));
