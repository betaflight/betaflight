...
const timerHardware_t bbTimerHardware[] = {
    DEF_TIM(ATIM1, CH1, NONE, 0, 0, 0),
    DEF_TIM(ATIM1, CH2, NONE, 0, 1, 0),
    DEF_TIM(ATIM1, CH3, NONE, 0, 2, 0),
    DEF_TIM(ATIM1, CH4, NONE, 0, 3, 0),
    DEF_TIM(ATIM2, CH1, NONE, 0, 0, 0),
    DEF_TIM(ATIM2, CH2, NONE, 0, 1, 0),
    DEF_TIM(ATIM2, CH3, NONE, 0, 2, 0),
    DEF_TIM(ATIM2, CH4, NONE, 0, 3, 0),
};
...
static void bbFindPacerTimer(void)
{
    for (int bbPortIndex = 0; bbPortIndex < MAX_SUPPORTED_MOTOR_PORTS; bbPortIndex++) {
        for (unsigned timerIndex = 0; timerIndex < ARRAYLEN(bbTimerHardware); timerIndex++) {
            ...
            if ((motorConfig()->dev.useDshotBitbangedTimer == DSHOT_BITBANGED_TIMER_ATIM1 && timNumber != 1)
                || (motorConfig()->dev.useDshotBitbangedTimer == DSHOT_BITBANGED_TIMER_ATIM2 && timNumber != 2)) {
                continue;
            }
...
static bool bbDecodeTelemetry(void)
{
    ...
    if (useDshotTelemetry) {
        ...
        for (int i = 0; i < usedMotorPorts; i++) {
            ...
            X32_INVALIDATE_DCACHE_BY_ADDR(bbPort->portInputBuffer, DSHOT_BB_PORT_IP_BUF_CACHE_ALIGN_BYTES);
        }
#endif
