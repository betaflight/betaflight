...
#if !defined(X32M7)
#error "EXTI is not defined for this X32 target"
#endif
...
static const uint32_t triggerLookupTable[] = {
...
static uint32_t extiLineMask(IO_t io)
{
    const int chIdx = IO_GPIOPinIdx(io);

    return (chIdx >= 0) ? (1U << chIdx) : 0;
}
...
{
    uint32_t extiLine = extiLineMask(io);
