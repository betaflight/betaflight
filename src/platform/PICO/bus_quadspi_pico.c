...
static inline bool qmi_direct_io(const uint8_t *tx, uint8_t *rx, size_t count, timeMs_t timeout)
{
    ...
    while (tx_remaining || rx_remaining) {
        ...
        if (cmpTimeMs(millis(), start_ms) > (int32_t)timeout) {
...
bool quadSpiReceive1LINE(QUADSPI_TypeDef *instance, uint8_t instruction, uint8_t dummyCycles, uint8_t *in, int length)
{
    ...
    while (dummyCycles >= QSPI_DUMMY_BITS_PER_BYTE) {
        uint8_t z = 0x00;
        if (!qmi_direct_io(&z, NULL, 1, QSPI_TIMEOUT_MS)) {
            qmi_timeout_cleanup();
            return false;
        }
    ...
    for (int i = 0; i < length; i++) {
        ...
        if (!qmi_direct_io(&tx, &rx, 1, QSPI_TIMEOUT_MS)) {
            qmi_timeout_cleanup();
            return false;
        }
        in[i] = rx;
...
static inline bool pico_qspi_send_addr(uint8_t instruction, uint32_t address, uint8_t addressSize, bool isRead, uint8_t dummyCycles)
{
    UNUSED(isRead);
                                    ...
                                    uint32_t address, uint8_t addressSize, uint8_t *in, int length)
{
    ...
    int idx = 0;
    hdr[idx++] = instruction;
    for (int i = (addressSize/8) - 1; i >= 0; i--) hdr[idx++] = (address >> (i*8)) & 0xFF;
    while (dummyCycles >= QSPI_DUMMY_BITS_PER_BYTE) {
        hdr[idx++] = 0x00;
        dummyCycles -= QSPI_DUMMY_BITS_PER_BYTE;
    }
    ...
    for (int i = 0; i < length; i++) {
        uint8_t tx = 0x00;
        if (!qmi_direct_io(&tx, &in[i], 1, QSPI_TIMEOUT_MS)) {
            qmi_timeout_cleanup();
            return false;
        }
    }
