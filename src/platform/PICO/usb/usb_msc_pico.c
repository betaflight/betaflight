...
static int8_t pico_sdspi_Read(uint8_t lun, uint8_t *buf, uint32_t blk_addr, uint16_t blk_len)
{
    UNUSED(lun);
...
static int8_t pico_sdspi_Write(uint8_t lun, uint8_t *buf, uint32_t blk_addr, uint16_t blk_len)
{
    UNUSED(lun);
    for (uint16_t i = 0; i < blk_len; i++) {
