...
FAST_CODE timeUs_t micros(void)
{
    ...
    if ((deltaTicks >> 32) == 0 && (uint32_t) deltaTicks <= UINT32_MAX - microsResidualTicks) {
        ...
        microsElapsedUs += pendingTicks / usPerTick;
        microsResidualTicks = pendingTicks % usPerTick;
    } else {
        ...
        microsElapsedUs += (timeUs_t) (deltaTicks / usPerTick);
        microsResidualTicks = (uint32_t) (deltaTicks % usPerTick);
    }
