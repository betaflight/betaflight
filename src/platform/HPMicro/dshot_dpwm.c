...
{
    // Leading pause: 4 bit-periods of idle level.  Must use stride so that
    // burst-DMA (stride ≥ 4) places these in the correct per-channel slots.
    for (int i = 0; i < 4; i++) {
    ...
    dmaBuffer[20 * stride] = dshot_duty_count << 4;
    dmaBuffer[21 * stride] = dshot_duty_count << 4;
