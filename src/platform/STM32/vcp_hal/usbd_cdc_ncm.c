...
static void ncmSendNextNotification(USBD_HandleTypeDef *pdev, bool forceNext)
{
    ...
    if (ncm.notifyState == NOTIFY_SPEED) {
        ...
        ncm.notifyBusy = 1;
        USBD_LL_Transmit(pdev, NCM_NOTIF_EP, ncm.notifBuf, 16);
        ...
        ncm.notifyBusy = 1;
        USBD_LL_Transmit(pdev, NCM_NOTIF_EP, ncm.notifBuf, 8);
    } else {
        ncm.notifyBusy = 0;
    }
}
...
static uint8_t ncmSetup(USBD_HandleTypeDef *pdev, USBD_SetupReqTypedef *req)
{
    ...
    case USB_REQ_TYPE_CLASS:
        ...
        case NCM_GET_NTB_INPUT_SIZE: {
            uint8_t buf[4];
            buf[0] = LOBYTE(NCM_NTB_IN_MAX_SIZE); buf[1] = HIBYTE(NCM_NTB_IN_MAX_SIZE);
            buf[2] = 0; buf[3] = 0;
            USBD_CtlSendData(pdev, buf, MIN(4, req->wLength));
            break;
        }

        case NCM_GET_NTB_FORMAT: {
            uint8_t buf[2] = { 0x00, 0x00 };   // 16-bit NTB
            USBD_CtlSendData(pdev, buf, MIN(2, req->wLength));
            break;
        }
        ...
        case NCM_SET_ETHERNET_PACKET_FILTER:
            if (req->wLength != 0 && (req->bmRequest & 0x80U) == 0) {
                ...
                // no data stage: host configured the filter, bring the link up
                if (req->bRequest == NCM_SET_ETHERNET_PACKET_FILTER && !ncm.linkUp) {
                    USBD_CDC_NCM_SetLinkState(pdev, 1);
                }
            }
            break;
    ...
    case USB_REQ_TYPE_STANDARD:
        ...
        case USB_REQ_GET_INTERFACE:
            if (pdev->dev_state == USBD_STATE_CONFIGURED) {
                ifalt = (req->wIndex == NCM_DATA_ITF) ? ncm.dataAltSet : 0;
...
static uint8_t ncmDataIn(USBD_HandleTypeDef *pdev, uint8_t epnum)
{
    if (epnum == (NCM_NOTIF_EP & 0x7FU)) {
        ...
        if (ncm.txLastLen != 0 && (ncm.txLastLen % NCM_DATA_FS_PACKET_SIZE) == 0) {
            ncm.txLastLen = 0;
            USBD_LL_Transmit(pdev, NCM_IN_EP, NULL, 0);   // length 0, NULL buffer is safe
        } else {
            ncm.txLastLen = 0;
            ncm.txBusy = 0;
        }
...
void USBD_CDC_NCM_Process(USBD_HandleTypeDef *pdev)
{
    ...
    if (ncm.rxReady) {
        ...
        if (ncm.fops && ncm.fops->Receive &&
            rxLen >= sizeof(nth16_t) + sizeof(ndp16_t) + 2U * sizeof(ndp16_datagram_t) &&
            nth->wHeaderLength == sizeof(nth16_t) &&
            nth->dwSignature == NTH16_SIGNATURE &&
            nth->wNdpIndex >= sizeof(nth16_t) &&
            (uint32_t)nth->wNdpIndex + sizeof(ndp16_t) + 2U * sizeof(ndp16_datagram_t) <= rxLen) {
            ...
            if (ndp->dwSignature == NDP16_SIGNATURE || ndp->dwSignature == NDP16_SIGNATURE_NCM1) {
                // only walk datagram pointers that actually fit inside the received bytes
                uint32_t entries = (rxLen - nth->wNdpIndex - sizeof(ndp16_t)) / sizeof(ndp16_datagram_t);
                ...
                for (uint32_t i = 0; i < entries && dg[i].wDatagramIndex != 0 && dg[i].wDatagramLength != 0; i++) {
                    if ((uint32_t)dg[i].wDatagramIndex + dg[i].wDatagramLength <= rxLen) {
...
uint8_t USBD_CDC_NCM_TransmitFrame(USBD_HandleTypeDef *pdev, const uint8_t *frame, uint16_t length)
{
    ...
    const uint16_t nthLen = sizeof(nth16_t);
    const uint16_t dgOffset = nthLen;
    const uint16_t ndpOffset = (uint16_t)((dgOffset + length + 3U) & ~3U);
    const uint16_t total = (uint16_t)(ndpOffset + sizeof(ndp16_t) + 2U * sizeof(ndp16_datagram_t));
    ...
    nth->dwSignature = NTH16_SIGNATURE;
    nth->wHeaderLength = nthLen;
    nth->wSequence = ncm.txSeq++;
    nth->wBlockLength = total;
    nth->wNdpIndex = ndpOffset;
    ...
    ncm.txBusy = 1;
    ncm.txLastLen = total;
    USBD_LL_Transmit(pdev, NCM_IN_EP, ncm.txNtb, total);
    return USBD_OK;
}
