...
// H5 has internal channels: VREFINT and TEMPSENSOR on ADC1, VBAT/4 on ADC2
#define ADC_DEVICE_FOR_INTERNAL_VREF ADC_DEVICES_1
#define ADC_DEVICE_FOR_INTERNAL_VBAT ADC_DEVICES_2

/* note these could be packed up for saving space */
const adcTagMap_t adcTagMap[] = {
#ifdef USE_ADC_INTERNAL
    // Pseudo entries for internal sensor.
    // Keep these at the beginning for easy indexing by ADC_TAG_MAP_{VREFINT,TEMPSENSOR,VBAT4}
#define ADC_TAG_MAP_VREFINT    0
#define ADC_TAG_MAP_TEMPSENSOR 1
#define ADC_TAG_MAP_VBAT4      2

    { DEFIO_TAG_E__NONE, ADC_DEVICE_FOR_INTERNAL_VREF, ADC_CHANNEL_VREFINT,    17 }, // ADC1 ch17 VREFINT
    { DEFIO_TAG_E__NONE, ADC_DEVICE_FOR_INTERNAL_VREF, ADC_CHANNEL_TEMPSENSOR, 16 }, // ADC1 ch16 VSENSE
    { DEFIO_TAG_E__NONE, ADC_DEVICE_FOR_INTERNAL_VBAT, ADC_CHANNEL_VBAT,       16 }, // ADC2 ch16 VBAT/4

#endif // USE_ADC_INTERNAL

    // STM32H5 ADC pin mappings (ADC1 and ADC2)
...
// H5 ADC sample times: 2.5, 6.5, 12.5, 24.5, 47.5, 92.5, 247.5, 640.5 cycles
...
static void adcInitDevice(adcDevice_t *adcdev, int channelCount)
{
    ...
    hadc->Init.DMAContinuousRequests    = ENABLE;                        // Circular DMA
...
void adcInit(const adcConfig_t *config)
{
    ...
    for (unsigned i = 0; i < ADC_SOURCE_COUNT; i++) {
        ...
        if (i < ADC_EXTERNAL_COUNT) {
            ...
            adcOperatingConfig[i].sampleTime = ADC_SAMPLETIME_247CYCLES_5;
...
void adcGetChannelValues(void)
{
    ...
    // H5 Cortex-M33 does not require SCB_InvalidateDCache_by_Addr like H7's Cortex-M7
